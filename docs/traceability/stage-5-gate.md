...
| AK 49 | Registry-Wirksamkeit | `crates/ea-admin/tests/registry_workflows.rs::the_workflow_selects_the_highest_applicable_head` und `::a_wrong_previous_head_blocks` (20/0/0), fuer Forks und future-only Heads `e2e_registry_effectiveness.rs` (7/0/0); den neueren wirksamen Head am Server erzwingen `apps/server/tests/auth_trust_api.rs::a_registry_event_that_is_not_the_next_head_names_the_head_that_is`, `::a_selected_head_behind_the_persisted_pin_is_refused` und `commit_failures.rs::a_package_binding_an_older_head_names_the_required_head` (mit `xtask integration up`, gemessen im `verify:quick`-Lauf) | Die unvermeidbare Offline-Grenze ist dokumentiert und bleibt bestehen |
...
**Go-live-Bericht ohne Einstufung zum Restnachweis — besitzendes Ticket
DRK-282.** AK 44 verlangt, dass der Go-live-Bericht Einstufung und
Entscheidung zum Restnachweis enthaelt. `GoLiveChecklist` in
`crates/ea-admin/src/go_live.rs` kennt keine solche Anforderung, und kein
Test belegt sie. Die externe Datenschutzentscheidung selbst bleibt Stufe 7;
dass der Bericht sie traegt, gehoert in diese Stufe. Bis dahin kann AK-44
nicht wandern.
