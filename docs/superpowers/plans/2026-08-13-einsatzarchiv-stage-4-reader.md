...
**Das Status-DTO wird in `crates/ea-reader-wasm` gebaut, und ein Wirtszeuge misst jedes seiner Felder.** Die erste Fassung verortete `file_mode_archive_json` in `crates/ea-reader/src/file_mode.rs`, „damit ein gewöhnlicher Wirtstest sie fahren kann; dieselbe Bauform wie `bundle_activation_json`". Der ORT ist gegen den Arbeitsbaum falsch, die BEGRÜNDUNG bleibt: `bundle_activation_json` liegt GEMESSEN in `crates/ea-reader-wasm/src/bridge.rs` und wird von einem gewöhnlichen Wirtstest im `#[cfg(test)] mod tests` derselben Datei gefahren, `crates/ea-reader/Cargo.toml` führt überhaupt keinen JSON-Weg, und der einzige JSON-Schreiber des Arbeitsbaums ist `pub(crate) struct Json` in `crates/ea-reader-wasm/src/bridge.rs` — eine zweite Fassung in `ea-reader` wäre der zweite JSON-Schreiber.

```rust
// crates/ea-reader-wasm/src/file_access.rs — die REINE Haelfte der zwei
// oeffnenden Ausfuhren, ohne cfg und damit auf jedem Ziel uebersetzbar.
#[must_use]
pub fn file_mode_archive_json(opened: &OpenedArchiveV1) -> String;
```

Diese Funktion ist die EINZIGE Strecke, auf der ein Befund der neun Gates in die Oberfläche gelangt: beide öffnenden Ausfuhren enden mit ihr, und `apps/web` liest danach nur noch das DTO. Ein Zeuge über der Faltungsregel allein misst sie NICHT — er steht an der Rechnung und nicht am Ergebnis, und eine Zuordnung, die `serverConfirmation` aus `fullyVerified` ableitete oder die zwei Zählspalten vertauschte, bliebe grün. Deshalb steht das Wirtsziel `crates/ea-reader-wasm/tests/file_mode_dto.rs` daneben, über derselben `#[path]`-Kulissenkette wie `tests/verify_browser.rs`, mit zwei Zeugen: `every_field_of_the_view_is_read_from_the_report_and_none_is_recomputed` prüft jedes der sieben Felder gegen die Berichtsmethode, aus der es kommen MUSS (und nicht gegen eine hier hingeschriebene Zahl), und `the_confirmation_column_is_not_a_reading_of_the_verification_state` stellt die zwei Dimensionen aus §17.4 GEGENLÄUFIG nebeneinander — der lückenlose Bestand ist vollständig verifiziert und `nicht server-bestätigt`, die Quittungslinie ist `server-bestätigt` und wegen ihrer Vorlauf-Lücke nicht vollständig verifiziert. Wer eine der zwei Spalten aus der anderen faltete, färbte diesen Zeugen an einem der zwei Bestände rot.
...
Der Durchlauf ist aber nicht die einzige Stelle dieser Datei, die etwas entscheidet: die FOLGE aus Anfangen, Durchlaufen, Ausfall-Vermerken und Öffnen ist es auch, und sie trägt die Zusicherung, ohne die ein halbes Archiv als ganzes durchginge — gefragt wird VOR und NACH dem Durchlauf, weil ein `FileSystemDirectoryHandle` den Entzug erst beim nächsten Zugriff meldet, und eine Abschneidung am Kettenende erzeugt keine Lücke, über die der Bericht stolperte. Über `fileModeBridge` allein ist diese Folge nur mit echtem Worker und wasm-Modul zu fahren und damit von keinem Zeugen erreichbar; sie steht deshalb als `openDirectoryOverPort(handle, session, call)` über einem `FileModeWorkerPort` — derselbe Zug wie `FileModePushPort` bei `walkDirectoryHandle`, eine Ebene höher. Die drei Zeugen dafür in `apps/web/src/features/file-mode/DirectoryHandle.test.ts` heissen `carries every file to the worker and then opens the directory it began` (mit der Positivkontrolle, dass dabei KEIN Ausfall gemeldet wird), `marks the folder unavailable when the permission is withdrawn during the walk` und `hands over not a single byte when the folder is already closed`.
...
- Modify: `apps/web/src/e2e-config.test.ts`
- Modify: `docs/traceability/stage-4-fault-points.json`
- Modify: `docs/traceability/v0.1-requirements.csv`
- Modify: `tools/xtask/src/main.rs`
- Modify: `tools/xtask/tests/stage_gate.rs`
...
**Gemessen am 2026-09-05:** `without_grant_for` darf KEIN Loeschen des `.eag` sein. Der Hash des initialen Grantplans ist in das signierte Manifest gebunden, Gate `grant-plan` rekonstruiert den Plan aus den vorhandenen Grantobjekten und haelt ihn dagegen (`crates/ea-verify/src/entry.rs`; die Planregeln in `validate_plan_items`, `crates/ea-format/src/eag.rs`). Wird Bs `.eag` physisch entfernt, endet der Eintrag fuer BEIDE Reader als `Invalid` mit `EA-VERIFY-GRANT-PLAN-MISMATCH` — auch fuer A, dessen Grant noch da ist. `MissingGrant` entsteht nur aus einem ZWEITEN Bau, dessen Plan B nie genannt hat, und genau das muss `without_grant_for` bedeuten (im Baum: `fixtures::archive_with_a_grant_for_reader_a_only()`; das Loeschen misst `removing_one_grant_object_is_a_plan_mismatch_for_both_readers_and_never_a_missing_grant` als das, was es ist).
...
fn stage_four_gate_requires_two_readers_the_browser_matrix_and_the_file_mode() {
    ...
    // GEMESSEN gegen `resolved_fault_point_witnesses` in `tools/xtask/src/main.rs`:
    // die Funktion sammelt in ein `BTreeSet<String>`, und ZWOELF Szenarien des
    // Abschnitts `sync-cursor` teilen EINEN Zeugen. Die Zahl ist deshalb die der
    // VERSCHIEDENEN Zeugen des Manifests und nicht die der Szenarien; eine
    // Gleichheit mit `STAGE_FOUR_SCENARIOS.len()` waere auf dem eingecheckten
    // Manifest rot, ohne dass ein Zeuge fehlte.
    assert_eq!(
        report["stage_four_fault_point_witnesses"].as_array().unwrap().len(),
        distinct_witnesses_in(&workspace_root(), STAGE_FOUR_FAULT_POINTS_PATH).len(),
    ...
    // `remove_fault_point_section` entsteht in DIESER Aufgabe: die Stufe 3
    // traegt denselben Schritt INLINE in Phase 3 von
    // `stage_three_gate_requires_real_service_failures_and_primary_ak`, und der
    // Helfer hebt ihn heraus, statt ihn ein zweites Mal abzuschreiben.
...
**Gemessen am 2026-09-05, gegen die ausgelieferte `apps/web`-Flaeche:** die Skizze oben ist in ihrer FORM nicht ausfuehrbar, ihre AUSSAGE bleibt. `openPinnedBundle` gibt es nicht; das Oeffnen eines Bestands in der Oberflaeche laeuft ueber `readerSession()` in `apps/web/src/features/file-mode/DirectoryHandle.ts`, also ueber eine WebAuthn-PRF-Zeremonie, und `WebAuthn.addVirtualAuthenticator` ist eine CDP-Methode — auf `firefox` und `webkit` gibt es damit KEINEN Oberflaechenweg zu einem Bericht. Die Test-IDs `report-hash` und `verification-status` existieren nicht (die Datei-Modus-Flaeche traegt `verification-summary` und `server-confirmation`), und kein DTO fuehrt ein Feld `reportHash`. `apps/web/tests/e2e/fixtures/pinned-bundle.hex` ist ein `webBundleRelease`-Kandidat von 49 Byte und kein Archiv. Der Zeuge fuehrt die Aussage deshalb dort, wo sie wirklich liegt — im geteilten Kern: `browser-matrix.spec.ts` instanziiert auf `/datei` im GEBAUTEN Buendel unter der CSP den eigenen Modul-Worker der Anwendung (`dist/assets/opfs-worker-<hash>.js`, genau ein Treffer), sendet `vault-unlock` (reines Rust, ohne WebAuthn) ueber eine eingefrorene versiegelte Tresordatei, dann `file-mode-open-bundle` ueber ein eingefrorenes Archivbuendel und `reader-stand-view`, und vergleicht die `ReaderStandView` BYTEGLEICH mit der eingefrorenen Datei UND ihren SHA-256 mit EINEM Literal. Der „reportHash" ist also der SHA-256 ueber das `ReaderStandView`-JSON, weil der Kern keinen anderen ausgibt. Drei eingefrorene Dateien kommen dazu (`sealed-vault.hex`, `complete-archive.eabundle.hex`, `complete-archive-stand-view.json`), erzeugt aus `crates/ea-reader/tests/verify_fixtures/` ueber den Bestand `complete_archive_with_a_genesis_plaintext()` — dem einzigen Fixture-Bestand mit schemagueltigem Klartext, dessen Eintrag also `verifiziert` traegt. Anti-Leerlauf: ein gekipptes Byte in der Mitte von `entries/000000000000_entry.eip` (Offset aus dem Containerindex gelesen, nicht fest verdrahtet) ergibt einen ANDEREN Bericht, der selbst als Literal gepinnt ist (`EA-FORMAT-SHAPE` an Gate `format`); der Kern liest die Bytes also nachweislich und faellt auf jeder Engine gleich.

Die Engines, ebenfalls gemessen: `browsers up` exportiert allein `CHROMEDRIVER_REMOTE` (`run_browsers_up` in `tools/xtask/src/main.rs`) und keinen Pfad, unter dem Playwright auf dem Wirt Engine-Baus faende; `pnpm web:e2e` laeuft auf dem WIRT. `chromium-1234` und `firefox-1538` laufen dort aus dem Playwright-Cache (Firefox aus dem gepinnten Abbild `mcr.microsoft.com/playwright:v1.62.1-noble` extrahiert, revisionsgleich). `webkit-2336` startet auf einem Wirt ohne die WebKit-Systembibliotheken nicht (gemessen: `libevent-2.1.so.7` fehlt); der Zeuge faehrt WebKit deshalb ueber einen `playwright run-server` IM gepinnten Abbild (`docker run --network host <abbild> npx -y playwright@1.62.1 run-server --port 3001 --host 127.0.0.1`), und `apps/web/playwright.config.ts` haengt das Projekt `webkit` GENAU DANN ueber `connectOptions` an `EA_WEBKIT_WS_ENDPOINT`, wenn die Variable gesetzt ist; ohne sie startet WebKit lokal, die CI-Form. Ob `browsers up` diesen Dienst kuenftig selbst startet und die Variable druckt, ist eine Entscheidung fuer die Aufgabe, die das Gate schliesst, und steht dort als offene Zeile.
...
Lauf (a): Server-Modus ueber den quittungstragenden Bestand. Lauf (b): dasselbe Ein-Datei-Buendel dieses Bestands — GEMESSEN am 2026-09-05 nicht aus `write_archive_bundle`, sondern aus dem Fixture-Containerkodierer `exported_bundle_bytes` (`crates/ea-reader/tests/verify_fixtures/fixtures.rs`): `write_archive_bundle` weist in Schritt 4 jeden Bestand ab, der nicht `is_fully_verified()` ist (`SourceNotFullyVerified`), und JEDER quittungstragende Fixture-Bestand traegt absichtlich die Vorlauf-Luecke `0..=1` (`RECEIPT_PRE_ENTRY_GAP_THROUGH_V1` in `crates/ea-verify/tests/support/mod.rs`, „Wer sie wegrepariert, macht die Fixtures unwahr"); der Stufe-2-Writer-Bestand verifiziert ebenfalls nicht vollstaendig, und die einzige lueckenfreie Quittungslinie liegt hinter dem echten Server. Die Weigerung ist als eigener Zeuge gepinnt (`write_archive_bundle_refuses_the_receipt_bearing_fixture_because_its_line_carries_a_gap`), damit ein spaeterer lueckenfreier Quittungsbestand den Lauf (b) auf den echten Exporteur zurueckhaengt. Verglichen werden `archiveObjectCount`, `chainHead`, die Menge der `objectResults` UND die Spalte `serverConfirmation` — sie ist in (b) IDENTISCH zu (a), und genau das belegt, dass Gate-Schritt 7 die mitgereisten Quittungen wirklich auswertet statt sie zu ignorieren. Lauf (c) zerfaellt aus demselben Grund in zwei Zeugen: ueber DEMSELBEN quittungstragenden Bestand mit vorenthaltenen `.esr` kippt allein die Spalte `serverConfirmation`, `gaps()` und `is_fully_verified()` bleiben mit (a) identisch; und ueber dem lueckenfreien `complete_archive` durch den ECHTEN Exporteur auf einem echten `LocalPathBackend` steht jedes Objekt auf `notServerConfirmed` UND `ObjectResultKindV1::Valid`, `gaps()` ist leer und `is_fully_verified()` bleibt wahr — die orthogonale Dimension senkt nichts. NUR Lauf (c) traegt die Ledgerzeile des Datei-Modus; (a) und (b) belegen die Interoperabilitaet, nicht die Ausweisung. Der Negativfall daneben ist das untergeschobene Archiv mit vollstaendiger eigener Vertrauenskette: gegen den gepinnten Anker endet der Lauf fail-closed an Gate `trust`, `objectResults` bleibt leer und `publicKeyThumbprints` bleibt leer. Beide Zeugen sind hier die SYSTEMweite Wiederholung; ihr primaerer Beleg liegt in den Aufgaben „Verifikation vor Entschlüsselung, fehlender Grant, Modusparameter und der Anchor, den nur der Vault liefert" und „Datei-Modus: Einzeldatei-Bündel, Verzeichnis-Handle, kein Cursor, `nicht server-bestätigt`", und dieser Bericht verweist darauf, statt die Aussage ein zweites Mal zu erheben.

**Die drei Achsen, die aus der Vorfassung unveraendert bleiben.**

Ein Writer-Chiffrat mit Grants fuer zwei verschiedene Reader-Zertifikate und -KEM-Schluessel wird repliziert und von beiden unabhaengig verifiziert und entschluesselt; wird einem der beiden sein Grant genommen, sieht NUR dieser `fehlender Grant` (gemessen: ein Bestand, dessen Grant-Plan diesen Reader nie nannte — ein entferntes Grant-Objekt ist fuer beide ein Planabgleichsfehler), und der Befund ist nie eine `Lücke`. `e2e_reader_sync_interruptions.rs` unterbricht jeden Punkt des Abschnitts `sync-cursor`, einschliesslich der zwei nur im Browser moeglichen — ein waehrend eines Batches geschlossener Tab und ein durch Storage-Eviction abgebrochener OPFS-Schreibvorgang, im Systemzeugen als Rust-Modelle (fallengelassener Dienst, `QuotaExceeded`-Speicher), dieselben wie in `sync_resume.rs`; der Browserlauf selbst ist nicht Teil dieses Zeugen — und belegt, dass der bestaetigte Cursor nach jedem Abbruch unveraendert bleibt und der Wiederholversuch idempotent auf denselben Kopf laeuft. GEMESSEN am 2026-09-05 mit EINER benannten Ausnahme, die der Zeuge festhaelt statt sie zu glaetten: nach dem abgewiesenen Fork (`refusal-leaves-the-cursor`, Rahmen `fork-at-the-head`) steht der Cursor, aber der konkurrierende, gueltig signierte Genesis-Eintrag liegt bereits im inhaltsadressierten Cache — `accept_batch` legt jedes hashkonsistente Objekt VOR `classify` ab, und `ReaderObjectCache` kennt kein Entfernen —, und jeder ehrliche Wiederholversuch endet erneut mit `EA-READER-CHAIN-FORK`; erst Cacheverlust plus `rebuild_from_genesis` erreicht den Referenzkopf. Ob `ea-reader` als `conflicting` quarantaenisierte Objekte kuenftig aus dem Cache nimmt, ist eine Entscheidung ausserhalb dieser Aufgabe und steht im Gate-Bericht unter `## Offen in spaeterer Stufe`. `ReaderBatchV1::new` ist aus `ea-system-tests` nicht erreichbar (`ea-sync-protocol` ist dort keine Dev-Kante; `ea-reader` re-exportiert nur `HttpMethod`), die Seiten des Fake-Servers werden deshalb mit `minicbor` in der Neunerform von `reader-batch-v1` geschrieben, und `ReaderBatchV1::decode` erzwingt Bytegleichheit der Rueckkodierung, sodass eine Drift als `EA-READER-PROTOCOL` rot wird statt still gruen. `privacy_canaries_reader.rs` sucht je fachlichem Feld GENAU EINEN Marker — ein gemeinsamer Marker fuer zwei Felder liesse offen, welches geleckt hat, dieselbe Regel, die `tests/ea-system-tests/tests/privacy_canaries_writer.rs` schon durchsetzt — mit `ea_testkit::contains_canary` in sieben Stroemen: den rohen OPFS-Bytes (Tresor, Cache, Zustandsspeicher, Indexblob), dem Service-Worker-Cache, den Zwischenablage-Haken, den strukturierten Logs, den Fehlerberichten, den Servermetadaten und der Telemetrie. GEMESSEN am 2026-09-05, und der Zeuge sagt es selbst, statt die sieben gleich zu behandeln: `ea-reader` fuehrt KEIN Protokollierungswerkzeug — der einzige strukturierte Log, den der Reader schreibt, ist das signierte lokale Auditprotokoll (`READER_AUDIT_LOG_BLOB_KEY_V1`), und genau das wird durchsucht. Der Index hat KEINEN Persistenzpfad: `ReaderSearch::seal` gibt Bytes zurueck, und keine Crate deklariert eine Blobadresse dafuer; der Zeuge legt ihn unter eine eigene Adresse und schreibt dazu, dass diese Adresse seine Setzung ist und kein Vertrag. Die Servermetadaten sind KEIN wirtsseitiger Strom, sondern ein Rust-Wert (`ReaderRequestV1`) und werden als echte Bytes gemessen. Nur drei Stroeme — Service-Worker-Cache, Zwischenablage und Telemetrie — haben im Baum keine Rust-Entsprechung; fuer sie ist der Beleg ein QUELLTEXTSCAN ueber `apps/web/src` und `crates/ea-reader-wasm/src`, und der Zeuge nennt ihn dreimal ausdruecklich so, statt eine Laufzeitmessung zu behaupten. Dazu die Positivkontrolle, ohne die die ganze Datei auch dann gruen waere, wenn die Marker nie ins System gelangt sind: derselbe Marker ist ueber den entsperrten Tresor lesbar, und die Suche findet ihn in einem absichtlich unverschluesselt abgelegten Kontrollstrom.

**Das Gate-Werkzeug.**

Der Stufenschalter in `run_stage_gate` (`tools/xtask/src/main.rs`) oeffnet mit `if stage == 4 { return run_stage_four_gate(root); }`, und seine Fehlermeldung zieht mit: heute `"stage-gate is only defined for stages 1, 2 and 3 so far, not {stage}"`, danach `"stage-gate is only defined for stages 1, 2, 3 and 4 so far, not {stage}"`. **Ein Test HAELT diese Zeichenkette, und die Vorfassung dieses Absatzes, die das Gegenteil behauptete, war gegen den Arbeitsbaum falsch** (gemessen am 2026-09-05): `the_stage_switch_still_refuses_an_undefined_stage` in `tools/xtask/tests/stage_gate.rs` faehrt den Schalter mit `"4"`, verlangt Exitcode 2 und den Teilstring `"stages 1, 2 and 3"`. Sein eigener Kopfkommentar sagt, er WANDERT mit dem Schalter: diese Aufgabe stellt ihn auf `"5"` und `"stages 1, 2, 3 and 4"`, in derselben Bewegung, mit der die Stufe-3-Abnahme ihn von 3 auf 4 gestellt hat. Der Grund, aus dem der Pin uebersehen wurde, steht hier ebenfalls: `grep -rn "only defined for stages"` trifft den Test NICHT, weil er den kuerzeren Teilstring `"stages 1, 2 and 3"` haelt; der Grep trifft eine Codestelle und FUENF Prosastellen (die Stufenplaene 2 und 3 sowie drei Saetze dieses Plans, darunter den in der Aufgabe „Verifikation vor Entschlüsselung, fehlender Grant, Modusparameter und der Anchor, den nur der Vault liefert", der „Kein Gate liest dieses Manifest fuer Stufe 4" sagt und mit dieser Aufgabe zur Aufzeichnung wird).
...
- `STAGE_FOUR_GATE_REPORT_SECTIONS: [&str; 10]` und `STAGE_FOUR_GATE_REPORT_LITERALS: [&str; 16]`, umlautfrei wie alle drei bereits geschlossenen Gate-Berichte, weil der Gate Literale vergleicht. ZEHN und nicht acht — die Vorfassung dieses Bullets nannte acht, und die Korrektur ist GEMESSEN und nicht kosmetisch. Die ersten acht, ausgeschrieben, weil eine noch fruehere Fassung nur vier davon nannte: `## 1. Primaere Abnahmekriterien und ihre Belege`, `## 2. Reichweite der Stufe-4-Abnahme`, `## 3. Fehlermatrix und deklarierte Szenarien`, `## 4. Entscheidungen dieser Stufe`, `## 5. Gemessene Indexschwelle`, `## Browsermatrix und Datei-Modus`, `## Rollengrenze`, `## Nicht beruehrte Nachbarzeilen`. DAZUGEKOMMEN sind `## Ledgerpflege` und `## Offen in spaeterer Stufe`, weil eine Mutation am 2026-09-05 gezeigt hat, dass BEIDE ganz loeschbar waren (39 beziehungsweise 21 Zeilen), ohne dass `pnpm stage-gate:4` etwas anderes als Exitcode 0 lieferte. Bei `## Offen in spaeterer Stufe` ist der Schnitt schaerfer als bei jedem anderen Abschnitt: der SECHZEHNTE Eintrag von `STAGE_FOUR_GATE_REPORT_LITERALS` — die Offenlegungspflicht dieser Stufe — endet auf „Alle vier stehen unten in `## Offen in spaeterer Stufe` mit ihrer besitzenden Stufe“, ZEIGT also auf genau diesen Abschnitt, und ein Zeiger, dessen Ziel loeschbar ist, waehrend der Zeiger gepinnt bleibt, ist keine Offenlegung. `## Ledgerpflege` steht aus dem Grund daneben, aus dem die drei geprueften Negative dabei sind: der Bericht sagt dort, WELCHE Ledgerzeilen die Stufe fortschreibt, und sein Schweigen liest sich als „keine“. `## Gemessener Gate-Lauf` bleibt dagegen wie in Stufe 3 UNGEPINNT, und das ist keine Nachlaessigkeit, sondern eine Notwendigkeit: den Abschnitt liest der benannte Test der Stufenabnahme (`stage_four_gate_report_records_the_measured_full_gate_run`), und er entsteht ERST, nachdem der Gate gelaufen ist — ein Gate, der ihn verlangte, koennte auf dem Lauf, der ihn erzeugt, nie gruen sein. Die Ledgertabelle steht in einem eigenen `## Ledgerpflege`; der Stufe-3-Bericht traegt seine Bewegung innerhalb von `## Nicht beruehrte Nachbarzeilen`, und dieser Bericht trennt beides, weil er den Abschnitt fuer die NICHT bewegten Zeilen braucht.
- **Die Abschnittspruefung ist ZEILENGENAU und nicht `contains`** — `require_document_sections` in `tools/xtask/src/main.rs` verlangt jeden Pflichtabschnitt als eigene, rechts getrimmte Zeile. Der Unterschied ist gemessen und tragend: die Zeichenkette `## Offen in spaeterer Stufe` steht im Stufe-4-Bericht SIEBENMAL, sechsmal davon als Verweis mitten im Fliesstext — einer davon in eben der Offenlegungspflicht, die als Pflichtliteral gepinnt ist. Ueber `contains` geprueft bediente dieser Zeiger also seinen EIGENEN Pflichtabschnitt: der Abschnitt liess sich am 2026-09-05 ganz herausschneiden, ohne dass `stage-gate 4` etwas anderes als Exitcode 0 lieferte, obwohl er in der Liste stand. Die Pflichtliterale bleiben absichtlich Teilketten und laufen weiter ueber `require_document_literals`: sie sind Begriffe im Fliesstext und haben keine Zeile fuer sich.
...
- `STAGE_FOUR_STEP_SIX_COMMANDS: [&str; 17]` in `tools/xtask/tests/stage_gate.rs`, nach dem Muster `STAGE_THREE_STEP_SIX_COMMANDS` (das dort `[&str; 8]` ist — „Muster" heisst Form, nicht Stelligkeit): die Kommandofolge aus Schritt 4, Wort fuer Wort und in Reihenfolge, mit `cargo metadata --format-version 1` an erster, `integration up` an zweiter und `integration down` an letzter Stelle. Die Dreizehn statt der Zwoelf war der Lockfile-Fortschritt dieses Tasks: die Dev-Kante `ea-reader` in `tests/ea-system-tests/Cargo.toml` schreibt `Cargo.lock` fort, und `integration up` traegt selbst `--locked`, kann also nicht davorstehen. Die Fuenfzehn statt der Dreizehn sind `browsers up` und `browsers down` aus ADR 0005 — dieser Lauf faehrt mit `pnpm web:browser-test` und `pnpm web:e2e` die zwei einzigen Kommandos der Stufe, die Engine-Baus und einen `chromedriver` voraussetzen, und beide beziehen sie aus dem gepinnten Abbild statt vom Wirt. Die Sechzehn statt der Fuenfzehn ist `pnpm index:scale`: die Aufgabe „Verschlüsselter invertierter Index in OPFS, Suche, Schemakompatibilität und die GEMESSENE 50.000-Paket-Schwelle" hat das Messwerkzeug ausgeliefert (`crates/ea-index/tests/scale_50000.rs`, `#[ignore]`, gefahren ueber `xtask index-scale 50000`), aber KEINE Messung persistiert — gemessen am 2026-09-05 nennt kein Dokument unter `docs/traceability/` einen Wert fuer `blob_bytes`, `unlock_ms` oder `peak_rss_kib`. Die Aufgabe „Verschlüsselter invertierter Index …" nennt sie sehr wohl, in ihrem eigenen Abschnitt dieses Plans und fuer beide Profile — aber ein Plantext ist kein Nachweisbestand, und der Gate-Bericht zeigt auf `docs/traceability/`. Der Abschnitt `## Gemessene Indexschwelle` kann deshalb nur aus einem Lauf DIESER Aufgabe gefuellt werden, und der Lauf steht im gemessenen Gate-Lauf, damit seine Zahlen eine Belegzeile mit Laufzeit tragen. Die Siebzehn statt der Sechzehn ist `pnpm build:wasm`, und sie ist GEMESSEN und nicht vorsorglich: `apps/web/src/bridge/opfs-worker.ts` importiert `./pkg/ea_reader_wasm.js`, und dieses Paket erzeugt allein `xtask build-wasm`. In der Vorfassung dieser Liste stand der Bau nur INNERHALB von `pnpm verify:quick` — also NACH `pnpm web:e2e`. In einem frischen Worktree faellt der Lauf damit an Kommando elf mit `[UNRESOLVED_IMPORT] Could not resolve './pkg/ea_reader_wasm.js'`, bevor ein Browser startet (gemessen am 2026-09-05, Exit 1 nach 3 s). Auf einem Baum, in dem irgendwann schon einmal `verify:quick` lief, waere es zufaellig durchgelaufen — genau die Reihenfolgenabhaengigkeit, die eine Kommandoliste ausschliessen soll. `pnpm build:wasm` steht deshalb direkt hinter `browsers up`: beide bereiten den Browserzweig vor. `pnpm web:browser-test` braucht das Paket NICHT (es uebersetzt seine eigenen `wasm-bindgen-test`-Ziele) und lief auch vorher gruen; der Fehler traf allein `web:e2e`. Die Stelligkeit waechst also aus VIER Gruenden in vier Ueberarbeitungen, und alle vier stehen hier.

**Der Lauf ist ZWEIPASSIG, und das ist eine Eigenschaft des Vertrags und kein Versehen.** `pnpm verify:quick` faehrt `cargo test --workspace --all-targets --locked`, und darin liegt `stage_four_gate_report_records_the_measured_full_gate_run`, der GENAU DIESE Messtabelle liest. Solange die Tabelle leer ist, ist `verify:quick` rot — gemessen: Exit 101, einziger Fehlschlag unter sonst gruenem Arbeitsbereich, mit der Meldung `stage-4-gate.md must record the measured run for ... exactly once`. Der Bootstraplauf erzeugt also die Zahlen, die ihn selbst gruen machen wuerden. Aufloesung, und der Bericht schreibt sie aus: die sechzehn uebrigen Zeilen tragen die Zahlen des Bootstraplaufs, die Zeile `pnpm verify:quick` traegt die des BESTAETIGUNGSLAUFS, der mit vollstaendiger Tabelle gefahren wurde. Eine Zeile, die den Bootstraplauf als gruen ausgaebe, waere die Faelschung, die dieses Repositorium nicht schreibt.
- Die LIVE-Zaehler kehren HIER zurueck, und nur hier. Die Aufgabe „wasm32-Reichweite: `ea-reader`, die Brücken-Crate und die geteilten Browserkerne" hat `verify_quick_subcommand_count()`, `wasm32_positive_list_count()` und `GERMAN_COUNT_WORDS` aus `tools/xtask/tests/stage_gate.rs` entfernt, weil ihre einzigen Aufrufstellen — die zwei ABGESCHLOSSENEN Berichte der Stufen 2 und 3 — auf historische Literale umgestellt wurden und ein ungenutzter Helfer `dead_code` und damit `cargo clippy … -- -D warnings` rot macht. `stage_four_gate_report_records_the_measured_full_gate_run` legt beide Zaehler wieder an — Zaehlung am zeichengenauen Pin `super::verify_quick_commands(),`, Teilkommandos ueber die Klammerbilanz, wasm32-Pakete ueber die `"-p"`-Vorkommen — und stellt sie gegen die Belegzeile `pnpm verify:quick` von `docs/traceability/stage-4-gate.md`. **Der wasm32-Zaehler darf dabei NICHT unveraendert zurueckkehren, und das ist gemessen:** sein alter Doc-Kommentar behauptete, die Positivliste sei „die einzige Stelle der Liste, die `-p` fuehrt". Das stimmte, bis `cargo run --locked -p xtask -- build-wasm` als fuenftes Teilkommando in `verify_quick_commands()` eintrat — es traegt sein eigenes `-p xtask`. Ueber den ganzen Pin gezaehlt ergibt die alte Fassung heute FUENFZEHN und wiese ein korrektes `VIERZEHN` ab. Der Zaehler beginnt deshalb am Literal `"wasm32-unknown-unknown"` und zaehlt nur bis zum Ende der Liste; die Begruendung steht an der Funktion und als Befund im Gate-Bericht. Die Zahlwortliste waechst dabei mit und wird HIER bemessen: die Stufe faehrt ZWOELF Teilkommandos und VIERZEHN wasm32-Pakete, der hoechste gebrauchte Index ist also 14, und die Liste ist `[&str; 15]` von `"NULL"` bis `"VIERZEHN"`. Die alte `[&str; 13]` deckte nur `0..=12` und haette an `get(13)` mit `None` PANIKT statt zu urteilen; wer sie spaeter weiter treibt, hebt die Stelligkeit in derselben Aufgabe, die den Zaehler treibt.
...
Ein Abschnitt `## 5. Gemessene Indexschwelle` — mit seiner Nummer, so wie ihn `STAGE_FOUR_GATE_REPORT_SECTIONS` pinnt; ein `## Gemessene Indexschwelle` ohne sie erfuellte die Konstante nicht — traegt die Zahlen, die `pnpm index:scale` im Lauf aus Schritt 4 DRUCKT — die Aufgabe „Verschlüsselter invertierter Index in OPFS, Suche, Schemakompatibilität und die GEMESSENE 50.000-Paket-Schwelle" hat das Werkzeug geliefert, keine persistierte Messung (siehe die Begruendung der sechzehnten Kommandozeile oben): Blobgroesse (`blob_bytes`), Entsperrlatenz (`unlock_ms`) und Spitzenspeicher (`peak_rss_kib`) bei 50000 indizierten PAKETEN, samt Profil (`debug`/`--release`) und der Angabe, ob warm oder kalt gemessen wurde. Die Zahl ist gebunden und nicht erfunden: `design.md` fordert in NFR-PERF-003 und Abnahmekriterium 31 „Ein Reader verifiziert und indiziert mindestens 50.000 Pakete", und die Stufe 7 misst genau diese Schwelle in `tests/ea-system-tests/tests/performance_reader_50000.rs`. Ein monolithischer Einzelblob unterhalb dieser Schwelle waere eine Stufe-4-Architektur, die ihr eigenes Stufe-7-Gate nicht bestehen kann. „Einsatz" und „Paket" sind dabei NICHT dieselbe Einheit — ein Einsatz traegt ein Original plus Nachtraege —, deshalb steht die Schwelle in Paketen, in derselben Einheit, die das Stufe-7-Gate misst. Der Bericht MISST und beansprucht nicht: die LEDGERZEILE `AK-31` behaelt `stage=7` und `status=planned`, und der Abschnitt sagt das ausdruecklich. `NFR-PERF-003` ist dabei ausdruecklich KEINE Ledgerzeile — der Bezeichner steht allein in der Anforderungstabelle von `docs/superpowers/specs/2026-08-13-einsatzarchiv-v0-1-design.md` und hat in `docs/traceability/v0.1-requirements.csv` keinen Eintrag; wer ihn dort zu drehen versucht, dreht nichts.
...
| `WR-041` | implemented | Web-Bundle: getrennter Origin, Service Worker, gepinnte `webBundleRelease` und das Alter des Trust-Standes | CODE-Seite; die betriebliche Haelfte steht in `## Offen in spaeterer Stufe`. `apps/web/src/sw/service-worker.test.ts` › „builds a bundle that addresses nothing absolutely and names no bundle origin" (relative Beiwerkspfade, `connect-src` ohne Bundle-Origin); ebenda › „pins the vite configuration that makes the separation possible" (`base: './'`, ungehashter Service-Worker-Einstieg) — Vitest-Titel mit Leerzeichen, keine Rust-Pfade; die Vorfassung schrieb sie in `snake_case`, und die Belegspalte des Ledgers muss die Titel tragen, die die Datei wirklich fuehrt |
...
| `WR-053` | integrated | Datei-Modus: Einzeldatei-Bündel, Verzeichnis-Handle, kein Cursor, `nicht server-bestätigt` | `crates/ea-reader/tests/file_mode_anchor.rs::a_substituted_archive_says_nothing_about_any_entry_in_file_mode` (so heisst die Funktion im Arbeitsbaum; die Vorfassung nannte einen Namen, den es nicht gibt); `tests/ea-system-tests/tests/reader_file_mode_interop.rs` |
...
Zwei Teilbelegzeilen entstehen nach dem Muster, das das Repositorium fuer `AK-19` und `AK-21` bereits fuehrt, jede `v1.1`, `stage=4`, `status=implemented`: „Keine Klartextlogs - Stufe-4-Teilbeleg (Reader)" fuer `AK-19` mit `tests/ea-system-tests/tests/privacy_canaries_reader.rs` als Beleg, und „Schema und Suite v1/v2 - Stufe-4-Teilbeleg (Reader-Altansicht)" fuer `AK-17` mit `crates/ea-index/tests/schema_compatibility.rs` — mit ASCII-Bindestrich, wie die vorhandenen Teilbelegzeilen `Keine Klartextlogs - Stufe-2-Teilbeleg` und `Backup-Restore - Stufe-3-Teilbeleg (Server-Restore)` ihn fuehren. Beide VOLLEN Zeilen behalten ihre bisherige Stufe. `AK-23` wird ausdruecklich NICHT beruehrt: der Plattform-Key-Provider ist Writer-Flaeche, und `web-reader-design.md` §11.4 nimmt die Achse Key-Provider fuer den Reader heraus, statt sie zu erfuellen.
