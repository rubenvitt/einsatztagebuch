...
- `STAGE_FOUR_GATE_REPORT_SECTIONS: [&str; 8]` und `STAGE_FOUR_GATE_REPORT_LITERALS: [&str; 16]`, umlautfrei wie alle drei bereits geschlossenen Gate-Berichte, weil der Gate Literale vergleicht. Die acht Abschnitte, ausgeschrieben, weil die Vorfassung nur vier davon nannte: `## 1. Primaere Abnahmekriterien und ihre Belege`, `## 2. Reichweite der Stufe-4-Abnahme`, `## 3. Fehlermatrix und deklarierte Szenarien`, `## 4. Entscheidungen dieser Stufe`, `## 5. Gemessene Indexschwelle`, `## Browsermatrix und Datei-Modus`, `## Rollengrenze`, `## Nicht beruehrte Nachbarzeilen`. `## Gemessener Gate-Lauf` und `## Offen in spaeterer Stufe` bleiben wie in Stufe 3 UNGEPINNT (der erste wird vom benannten Test der Stufenabnahme gelesen, nicht vom Gate). Die Ledgertabelle unten steht in einem eigenen, ungepinnten `## Ledgerpflege`; der Stufe-3-Bericht traegt seine Bewegung innerhalb von `## Nicht beruehrte Nachbarzeilen`, und dieser Bericht trennt beides, weil er den Abschnitt fuer die NICHT bewegten Zeilen braucht.
