...
`## Endpunkt- und Signaturabdeckung` with three measured statements: (a) the endpoint list of this plan is byte-identical with `docs/superpowers/specs/2026-08-13-einsatzarchiv-v0-1-design.md` §13.2 (:1516-1532) — SEVENTEEN lines on each side, and a `diff` produces no output; (b) the signature coverage of this plan covers design.md:1501-1507 position for position, with no missing and no additional position; (c) the three endpoints this stage adds — `POST /v1/webauthn-credentials`, `PUT /v1/vault-blobs` and `POST /v1/vault-blobs/retrievals` — are exactly the three §13.2 already carries, so the gate re-measures an equality that already holds instead of establishing it. Cost note, measured: no test, no schema and no code pins any of the seventeen paths — §13.2 carries no closing clause — so the three new endpoints cost exactly the two document pages and nothing else.

`## Serverhälften fremder Stufen` with three rows. AK-35 (design.md:2151, ledger `stage=5`) and AK-49 (design.md:2165, ledger `stage=5`) come into existence in the task „Atomic Entry Commit, Idempotent Replay, and Immutable Receipts“ („choose the highest server-known applicable Registry head … reject an older bound head“); AK-43 (design.md:2159, ledger `stage=4`) comes into existence as to its server half in the task „Reader, Object, Export, Historical-Grant, and Destruction API Surfaces“. This plan claims none of these rows and MUST NOT: their stage columns stay unchanged. The purpose is solely that Stages 4 and 5 find the server half already built. The proving test path is recorded in the same three rows as soon as it exists.
...
WR-042 additionally gets its evidence column corrected in the same movement — independently of the split, because that column cites a task of THIS plan that does not exist. The carrier field `reader-trust-refresh-ms` exists in `policy-core-v1` after decision D2 and is ALREADY frozen positively in Stage 1 by the task „Permanent Vectors, Property/Fuzz Gates, Format Package, and Traceability“: the two accepted vectors `vectors/trust/v1/object/accepted-policy-core-reader-trust-refresh-set/` and `vectors/trust/v1/object/accepted-policy-core-reader-trust-refresh-disabled/` — each a DIRECTORY carrying exactly one `policy.bin` — pinned against the CDDL by `trust_cddl_enforces_the_exact_twenty_two_positions_of_policy_core_v1` in `tools/xtask/tests/spec_completeness.rs`. The Stage-3 promise of the row thereby shrinks to what is really left open, the Service-Worker pinning, which the split above moves to Stage 4. The pin moves WITH the ledger in the same commit: in `web_reader_must_requirements_are_recorded_as_v1_1_rows`, beside the `WEB_READER_MUST_ROWS` loop, the `refresh` assertion today demands `contains("Task 10")` and now demands the frozen-vector wording `contains("accepted-policy-core-reader-trust-refresh")` instead, with the two comment lines above it rewritten accordingly, so that evidence column and pin cannot drift apart.
