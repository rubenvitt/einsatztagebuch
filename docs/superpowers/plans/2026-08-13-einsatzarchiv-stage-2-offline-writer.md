...
- Plain SQLite with per-record AEAD, rejected because `design.md:1966` requires full database encryption; per-record AEAD leaves the write-ahead log, all indexes and every temporary spill file readable, and the additional per-draft keys of the same sentence are a supplement to full encryption, not a substitute for it.
...
Task 15 gates the Writer only. Administration is Stage 5 (`design.md:2182`), and the Reader is the browser PWA (`2026-08-15-einsatzarchiv-web-reader-design.md:12-13`, `:54-56`) — the desktop shell therefore carries no Reader route, no Reader view, and no Reader command, and the role-gate test asserts that absence positively. This sentence exists so no later task re-adds either surface.
