...
policy-core-v1 = [
  ...
  reader-trust-refresh-ms: uint, ; device-side refresh deadline, 0 = unset
...
**Drei Inventarklassen, verbindlich** (normativ in `design.md` §11.4). Das Präfix entscheidet, nie der Dateiname: Bytes mit Exact-Object-Präfix sind Archivobjekte (Parse-Fehlschlag ⇒ Quarantäne mit Grund), Bytes ohne dieses Präfix sind *kein Archivobjekt* und zählen ausschließlich in `nonObjectFileCount`. Der Trust Anchor kommt als Parameter, nie aus dem Bestand. Ohne diese Trennung isolierte jedes normkonforme Archiv sein eigenes `README-FORMAT.txt` und wäre nie vollständig verifiziert.

**Ergebnisform der Kettenbildung, verbindlich.** `build_chain` liefert bei einem Fork `Ok(VerifiedChain)`, nicht `Err`. `VerifiedChain` trägt `forks()` neben `gaps()`; `verified_head()` ist die letzte unstrittige Sequenz, `head()` die höchste gesehene. Ein nicht leeres `forks()` macht `is_fully_verified()` false. Grund: ein `Err` kann die abgeschnittene Kette nicht mitführen, und ein Fork darf weder stillschweigend zur Lücke degradieren noch die Verifikation der unstrittigen Präfixsequenzen verhindern.

**Diagnostizierbarkeit, verbindlich.** `ea-types` leitet für Identitäts- und Hash-Typen kein `Debug` ab (`crates/ea-types/src/ids.rs:3`, `:27`, `:64`, `:86`, `:119`). `ea-chain` implementiert `Debug` für `ChainNode`, `ChainGap`, `ChainFork`, `ChainHead`, `VerifiedChain` und `ChainError` daher von Hand mit Hex-Ausgabe der Hashes — `ea-types` bleibt unberührt. `ChainError` bekommt zusätzlich `code() -> &'static str` nach dem Muster von `TrustSourceError::code` (`crates/ea-trust/src/error.rs:176-186`), damit Tests gegen stabile Codes assertieren statt gegen Formatierung.
...
**`stage-gate` ist eine stufenübergreifende Schnittstelle, kein Schreibfehler.** Der Subcommand fehlt heute im Dispatcher (`tools/xtask/src/main.rs:851-881`); die Gate-Kommandozeile `cargo run --locked -p xtask -- stage-gate 1` in Schritt 4 BLEIBT dennoch unverändert stehen, und dieser Task baut den Subcommand. Begründung, empirisch belegt: `docs/superpowers/plans/2026-08-13-einsatzarchiv-stage-4-reader.md:497`, `...stage-5-administration-recovery.md:874` und `...stage-6-evidence-grade.md:372` deklarieren jeweils `Produces: xtask stage-gate N`. Die Kommandozeile wegzukorrigieren würde also drei spätere Stufenpläne entwerten, statt einen Tippfehler zu beheben. Der Gate schreibt deterministisches JSON auf stdout; `tools/xtask/tests/stage_gate.rs` treibt das Binär über `CARGO_BIN_EXE_xtask` — exakt das Idiom von `tools/xtask/tests/schema_validation.rs`. Die Pseudocode-Form `xtask_test::stage_gate(1)` in Schritt 1 ist Skizze, kein Schnittstellenvertrag: `tools/xtask/tests/workspace.rs:176-243` liest `tools/xtask/src/main.rs` ALS TEXT, um `verify_quick_commands()` und `WASM32_EXEMPT_CRATES` zu parsen; ein Refactoring dieser Elemente in eine lib-Crate würde jenen Test still entwerten.
...
<!-- web-reader-blockers -->
**ENTSCHIEDEN — Formentscheidung nach `web-reader-design.md` §7.5.** Die menschliche Entscheidung vom 2026-08-17 legt fest: für die Zwei-Personen-Freigabe nach Spec §7.5 entsteht eine EIGENE 2-of-N-Trust-Objektfamilie nach dem Vorbild von `grantAuthorization`/`destructionAuthorization` (`schemas/archive/v1/trust.cddl:28-30`, `[2* cose-sign1-v1]`), gebaut in Stage 5 als v1.1. `organizationAdminAuthorization` bleibt UNVERÄNDERT bei Kardinalität 1 (`trust.cddl:22`, `[cose-sign1-v1]`, hart indiziertes `signatures()[0]` in `crates/ea-trust/src/admin_authorization.rs:142-149`) und beim 15-Feld-Array (`crates/ea-format/src/etb.rs:662`). Dieser Task DARF Positiv- UND Negativvektoren für `organizationAdminAuthorization` einfrieren. AUSDRÜCKLICH festgehalten: die Bindung eines Ziel-Transport-Public-Key-Fingerprints aus Spec §7.5 wird von dieser Familie NICHT ausgedrückt — Position 15 ist ein an drei Stellen auf Länge 0 geprüftes leeres Extension-Array (`crates/ea-format/src/etb.rs:676`, `:1489`, `crates/ea-crypto/src/cose.rs:2781`). Die hier eingefrorenen Vektoren sind deshalb KEIN Beleg für Spec §7.5; der Beleg entsteht erst mit der neuen Familie in Stage 5.

**ENTSCHIEDEN — Zuordnung der Policy-Frist nach `web-reader-design.md` §4.2.** Die menschliche Entscheidung vom 2026-08-17 hält fest: `policy-core-v1` bekommt ein EIGENES Feld für die geräteseitige Aktualisierungsfrist. Weder `max_registry_age_ms` (Ausstellungsschranke am Registry-Ereignis, `design.md:1367`) noch `registry_expiry_behavior` (an die Finalisierung gebunden, `design.md:1446` — eine Operation, die der Reader nicht ausführt) drückt sie aus. Umgesetzt VOR diesem Task: `reader-trust-refresh-ms: uint` steht unmittelbar nach `reader-inactivity-ms` (`schemas/archive/v1/trust.cddl`, `PolicyFieldsV1` in `crates/ea-format/src/etb.rs`); das geschlossene Array hat damit 22 statt 21 Positionen, und `parse_policy_core` in `crates/ea-crypto/src/cose.rs` zieht positionsgleich mit. Semantik normativ in `design.md` §12.3: nach dieser Zeitspanne seit dem letzten Bezug des Trust-Bestandes MUSS die Anwendung zur Aktualisierung auffordern; `0` bedeutet keine geräteseitige Frist. Dieser Task DARF seine Positivvektoren für `policy-core-v1` deshalb einfrieren.

**ENTSCHIEDEN — Traceability der Web-Reader-Anforderungen.** Die menschliche Entscheidung vom 2026-08-17 legt fest: die sieben MUSS-Anforderungen aus `docs/superpowers/specs/2026-08-15-einsatzarchiv-web-reader-design.md` werden als v1.1-Zeilen ins Requirement-Ledger AUFGENOMMEN, nicht zurückgestellt. Es sind §4.1 getrennter Origin, §4.2 Aktivierung nur gegen eine gepinnte, Root-signierte `webBundleRelease`, §4.3 nicht überspringbarer Fingerprint-Vergleich, §5.2 universeller Datei-Weg immer angeboten, §6.3 zwei Pflicht-Authenticators, §7.5 Verweigerung der Re-Encryption bei abweichendem Transport-Fingerprint und §8.2 kein Klartext in Telemetrie. Zusätzlich werden `design.md` §27.1 FR-100 („gemeinsame App, signierte Rollentrennung") und FR-103 („Reader-Cache und Index verschlüsselt") auf die neue Rollenaufteilung — Writer und Administration auf dem Desktop, Reader im Browser — nachgezogen. Die Ausführung beider Punkte gehört in Task 6 dieses Vorhabens; die Ledger-Zeilen tragen Status `planned` mit Verweis auf die bauende Stufe, weil Stage 1 keine dieser Anforderungen implementiert.

**ENTSCHIEDEN — Ablageort des Escrow-Chiffrats nach `web-reader-design.md` §7.3.** Die menschliche Entscheidung vom 2026-08-28 (DRK-213) legt fest: das Escrow-Chiffrat des Reader-KEM-Schlüssels liegt als EIGENE Root-signierte Trust-Objektart `readerKeyEscrow` im Trust-Bestand `trust/` des Archivs (`design.md` §11.4) und wird wie jedes Trust-Objekt an alle Reader repliziert und exportiert. Der undefinierte Begriff „Administrationszone" ist aus Spec §7.3 gestrichen. Bedrohungsanalyse: öffnen kann das Chiffrat nur der Recovery-KEM-Schlüssel, der per Invariante bereits den replizierten Recovery-Grant jedes Eintrags entkapselt; der Ed25519-Schlüssel wird nicht hinterlegt (§7.2); Zertifikat-Hash und `subjectId` stehen ohnehin im Reader-Zertifikat — die Replikation schafft keine neue Fähigkeit. Verworfen: ein opaker Server-Blob (nicht Root-signiert, nicht exportiert, bei Serververlust weg) und ein eigener Admin-Bestand außerhalb des Exports (neues Konzept, unterläuft die serverunabhängige Wiederherstellung). Gebaut wird die Objektart in Stage 5 als v1.1 im selben Cutover wie die 2-of-N-Familie; Stage 1 friert für sie NICHTS ein.
