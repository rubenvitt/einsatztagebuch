...
- Standard-profile stale-Registry continuation requires a non-bypassable warning, fresh re-authentication, and a durable signed one-use audit acknowledgement; strict/Evidence/lease failures remain blocked.
  - **Fußnote 2026-08-28 (Ruling R62), der Wortlaut oben bleibt unverändert:** Stufe 2 hat von diesem Punkt nur die ERKENNUNG mit fail-closed-Ausgang geliefert (`crates/ea-writer/tests/stale_registry_warning.rs::a_head_that_expires_while_bound_is_acknowledgeable_and_blocks_fail_closed`, `::an_overdue_refresh_deadline_warns_without_blocking`). Die dauerhafte signierte Einmal-Quittung ist NICHT gebaut — `WriterService::acknowledge_stale_registry` existiert nicht, und der Wirtsstummel `writer_acknowledge_stale_registry` (`apps/desktop/src-tauri/src/commands/writer.rs`) meldet `STALE_ACK_UNAVAILABLE`. Sie wandert ausdrücklich in Stufe 5, wo die Administrationsseite von AK 24 steht (`docs/superpowers/plans/2026-08-13-einsatzarchiv-stage-5-administration-recovery.md`). Belege und Begründung: `docs/traceability/stage-2-nacharbeit-2026-08-28.md`.
