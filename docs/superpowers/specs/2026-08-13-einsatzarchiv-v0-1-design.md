...
Der Administrator darf Anträge ablehnen, Root-Signaturen kontrolliert einspielen und die resultierenden Trust-Objekte verteilen. Seine Rolle enthält keine automatische Inhaltsentschlüsselung, Writer-, Grant- oder Server-Belegsignatur. Jede Root-Zeremonie nach dem Bootstrap, die **registrywirksamen** Trust-Zustand ändert, bindet den Hash des in Abschnitt 11.1 definierten `organizationAdminAuthorization`-Objekts und wird lokal auditiert. Direkte, wurzelsignierte Objektarten **ohne** Registry-Wirkung sind abschließend aufgezählt und binden keine Admin-Autorisierung: `webBundleRelease`, `webBundleRevocation` und — mit dem v1.1-Cutover — `readerKeyEscrow` (Korrektur vom 2026-09-22, DRK-318: der frühere Pauschalsatz war seit `webBundleRelease` unzutreffend; die Publikationsfreigabe des Escrows ist die eigene Familie `readerKeyEscrowApproval`). Einzige Ausnahme der Anfangszeremonie sind initiales Root-Zertifikat und mindestens zwei initiale Admin-Zertifikat-/Operator-Binding-Paare; ihre exakten Hashes und Paarungen werden vor der ersten Admin-Autorisierung im unabhängigen Recovery-Trust-Anchor aus Abschnitt 16.1 gepinnt.
...
Der stabile Kontobezeichner ist gerätelokal Windows-SID, macOS-Verzeichnis-ID plus UID oder Linux-Machine-ID plus UID; archiviert wird ausschließlich `SHA-256("EINSATZARCHIV-OS-ACCOUNT-v1" || deterministicCbor(os-account-context-v1))`. `os-account-context-v1` und die geschlossene Union `canonical-os-account-id-v1` sind bytegenau in `schemas/identity/v1/os-account.cddl` und im Wire-Format-Addendum festgelegt. Windows verwendet die validierte binäre `TokenUser`-SID, macOS die in 16 Netzwerk-Byte-Order-Oktette dekodierte 128-Bit-Open-Directory-GUID plus numerische UID und Linux die 16 dekodierten Machine-ID-Oktette plus numerische UID. Plattformstrings, Namen, frei gewählte Trennzeichen, Text-UIDs und normalisierte Laufzeitwerte sind kein zulässiger CBOR-Input. Die Anwendung liest alle Teile atomar für das aktuelle Konto über den Plattform-Key-Provider und darf sie nicht durch ein editierbares Profilfeld ersetzen.
...
`.etb` besitzt die signierten v1-Subtypen `rootCertificate`, `deviceCertificate`, `operatorBinding`, `organizationAdminAuthorization`, `registryEvent`, `policy`, `writerTransition`, `grantAuthorization`, `destructionAuthorization`, `destructionTransition` und `deletionAttestation`. Hinzugekommen sind seit Stufe 4 `webBundleRelease` und `webBundleRevocation` (normativ in `schemas/archive/v1/trust.cddl`); der v1.1-Cutover fügt `readerKeyEscrow`, `readerKeyEscrowApproval` und `readerKeyEscrowRecoveryAuthorization` hinzu. Maßgeblich ist `trust-subtype-v1` in `schemas/archive/v1/trust.cddl`, nicht diese Aufzählung. Der gemeinsame Signaturinput lautet:
