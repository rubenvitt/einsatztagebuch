...
| [`web-sys`](https://crates.io/crates/web-sys/0.3.103) | `=0.3.103`; `default-features = false`, twenty-nine named features | The [wasm-bindgen upstream](https://github.com/wasm-bindgen/wasm-bindgen/tree/master/crates/web-sys) is a procedurally generated binding crate over the whole WebIDL surface of the Web platform: over 1700 features exist, one per interface, and enabling none of them by default is the crate's own design. *Enumerated web-sys features* below names each of the twenty-nine this class turns on and the specification clause it exists for; nothing wider is admitted, and a thirtieth feature must pass through this gate. |
...
- **A `web-sys` pin without an explicit feature list (or with `--all-features`)**,
  ...
  twenty-nine admitted APIs and the specification clause each exists to serve.
...
names the twenty-nine this class turns on.
...
web-sys = ["Blob", "Crypto", "DedicatedWorkerGlobalScope", "Document", "Event", "File", "FileSystemDirectoryHandle", "FileSystemFileHandle", "FileSystemGetDirectoryOptions", "FileSystemGetFileOptions", "FileSystemReadWriteOptions", "FileSystemSyncAccessHandle", "Headers", "MessageEvent", "Navigator", "Performance", "Request", "RequestInit", "Response", "ServiceWorkerGlobalScope", "StorageManager", "SubtleCrypto", "VisibilityState", "Window", "Worker", "WorkerGlobalScope", "WorkerNavigator", "XmlHttpRequest", "XmlHttpRequestResponseType"]
```
...
interface; leaving it unlisted leaves that binding uncompiled. The twenty-nine
...
| `Performance` | `WorkerGlobalScope.performance` and its monotonic `now()`, nothing else of the interface | §3, the managed-replica cache removal (ruling 2026-09-13): the Reader's `ActionClock` (`crates/ea-reader-wasm/src/destruction_bridge.rs`) is the browser counterpart of the native `head_effective_now + opened.elapsed`. Time has to keep advancing across the OPFS waits between the caller's effective start (`effectiveNowMs`, raised to the verified floor through `max`) and the job-bound deletion attestation, and a wall clock gives no bound on elapsed time. A missing `performance` or a non-finite value refuses with `EA-READER-DESTRUCTION-CLOCK`. The interface's only WebIDL prerequisite, `EventTarget`, is already enabled through `WorkerGlobalScope`; its unused stable methods compile along because `web-sys` cuts per interface, not per method |

Twenty-nine admitted features and no others: a thirtieth must be justified
...
**Addendum 2026-09-14 — `Performance` admitted as the twenty-ninth feature.**
The Web-Reader design §3 was made precise on 2026-09-13: the Reader may remove
its own cache on a signed destruction order and sign its own job-bound deletion
attestation (DRK-250). The attestation's `executed_at` is derived from the
monotonic clock in the row above. The raw `performance.now()` value never
leaves the Reader, and the derived millisecond timestamp carries no more than
the `Date.now()` the TypeScript bridge already passes in. An independent review
on 2026-09-14 ratified the selection: the resolved tree gains no package, no
license and no further `web-sys` feature, and no unstable API is used.
