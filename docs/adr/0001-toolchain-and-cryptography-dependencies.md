...
| Fact | Consequence |
|---|---|
| `cryptoki` loads a module at runtime through `cryptoki-sys` and `libloading`. | That is the native-toolchain edge this ADR rejects for OpenSSL and `ring` under "Rejected alternatives": a `dlopen` boundary whose behaviour depends on the host's loader, its library search path and the module's own build. |
