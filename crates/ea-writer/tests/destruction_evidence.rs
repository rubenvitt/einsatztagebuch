...
#[test]
fn destruction_evidence_uses_normal_writer_preview_encryption_grants_and_irreversible_boundary() {
    run(false);
}
#[test]
fn destruction_evidence_reopens_after_key_absence_using_identical_prepared_bytes() {
    run(true);
}
fn run(recover: bool) {
    ...
    let reinjected = fixture.backend.root().join("entries/reinjected.eip");
    ...
    for (path, bytes) in [
        (
            "entries/quarantine/nested.eip.staging",
            &fixture.original.original_bytes,
        ),
        (
            "grants/quarantine/nested.eag.staging",
            &fixture.original.initial_grant_bytes,
        ),
    ] {
        let path = fixture.backend.root().join(path);
    ...
    #[cfg(unix)]
    {
        let link = fixture
            .backend
            .root()
            .join("entries/unreadable-custody.eip.staging");
        std::os::unix::fs::symlink("missing-managed-copy.eip", &link).unwrap();
        assert!(matches!(
            service.preview_destruction_evidence(&proof, input(), UnixMillis::new(1001)),
            Err(ea_writer::WriterError::DestructionEvidenceInvalid)
        ));
    ...
    let damaged = fixture
        .backend
        .root()
        .join("entries/quarantine/damaged.eip.staging");
    std::fs::write(
        &damaged,
        &fixture.original.original_bytes[..fixture.original.original_bytes.len() - 1],
    )
    .unwrap();
    assert!(
        matches!(
        ...
        "damaged actual object bytes cannot prove target absence"
    );
    ...
    let exact_recovered = if recover {
        ...
        let repository = Arc::new(repository.for_evidence_draft(binding).unwrap());
        let resumed = ea_writer::WriterService::new(
            repository,
            provider.clone(),
            &fixture.backend,
            &source,
            ...
            OperatorProfileRepository::new(reopened),
            ea_writer::WriterBindingV1 {
                binding_object_hash: fixture.native.binding,
                writer_certificate_hash: fixture.native.certificate,
                writer_key_thumbprint: fixture
        ...
        let reinjected = fixture
            .backend
            .root()
            .join("entries/crash-window-reinjected.eip");
        std::fs::write(&reinjected, &fixture.original.original_bytes).unwrap();
        assert!(
            matches!(
            ...
            "prepared recovery must preserve the evidence source fence"
        );
        assert_eq!(
            ea_archive::ArchiveInventory::build(&source)
                ...
                .len(),
            1,
            "only the re-injected original exists; prepared Evidence is not published"
        );
        std::fs::remove_file(reinjected).unwrap();
        let grant = fixture
            .backend
        ...
        for (path, bytes) in [
            (
                "entries/quarantine/restart-copy.eip.staging",
                &fixture.original.original_bytes,
            ),
            (
                "grants/quarantine/restart-copy.eag.staging",
                &fixture.original.initial_grant_bytes,
            ),
        ] {
            let path = fixture.backend.root().join(path);
        ...
        let damaged = fixture
            .backend
            .root()
            .join("grants/quarantine/damaged.eag.staging");
        std::fs::write(
            &damaged,
            &fixture.original.initial_grant_bytes[..fixture.original.initial_grant_bytes.len() - 1],
        )
        .unwrap();
        assert!(matches!(
            resumed.recover_pending(),
...
#[path = "destruction_evidence/draft_fence.rs"]
mod draft_fence;

#[path = "destruction_evidence/recovery_binding.rs"]
