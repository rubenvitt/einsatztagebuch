...
fn evidence_preview_refuses_an_existing_incident_draft_without_consuming_it() {
    ...
    let signing = provider.generate(SecretPurpose::WriterSigningKey, KeyProtectionProfileV1::OsWrapped).unwrap();
    let repository = Arc::new(AutosaveDraftRepository::new(fixture.native.database.clone(), provider.clone()));
    let before = repository.load_or_create().unwrap();
    let id = before.draft_id();
    let saved = repository.save(before.with_notes("existing incident MUST survive")).unwrap();
    ...
    let service = ea_writer::WriterService::new(
        repository.clone(), provider.clone(), &fixture.backend, &source, &fixture.head, &[],
        ...
        ea_writer::WriterBindingV1 {
            ...
            writer_key_thumbprint: fixture.head.active_certificate_fields(fixture.native.certificate).unwrap().signing_key_thumbprint.unwrap(),
            writer_signing_handle: signing,
            chain_id: fixture.head.chain_id(),
            archive_profile_hash: fixture.backend.profile_hash().unwrap(),
        },
    );
    let proof = fixture.native.proof(&fixture.head, ea_operator::ReauthPurpose::Finalize);
    let result = service.preview_destruction_evidence(&proof, ea_writer::DestructionEvidenceInputV1 {
        timezone: "Europe/Berlin".into(),
        source: ea_schema::NativeSourceV1::new("ea.native", 1).unwrap(),
        evidence: fixture.evidence.clone(),
    }, UnixMillis::new(1001));
    assert!(result.is_err(), "evidence must not use the active incident draft");
...
fn evidence_reservation_survives_reopen_and_blocks_all_ordinary_draft_access() {
    ...
    let repository = AutosaveDraftRepository::new(fixture.native.database.clone(), provider.clone());
    ...
    let binding = repository.reserve_evidence_draft(source).expect("a real started immutable job can reserve the empty draft");
    ...
    assert!(reopened.load_or_create().is_err(), "WriterPage must not see an apparently blank Incident");
    ...
    assert!(reopened.commit_discard_intent(&saved).is_err(), "ordinary discard cannot resolve a referenced Evidence");
}
...
fn bound_evidence_draft_cannot_be_finalized_as_an_incident() {
    ...
    use ea_local_store::StoreValue as V;
    fixture.native.database.execute("INSERT INTO operator_profile(singleton,organization_id,operator_subject_id,display_name,function_label,profile_commitment_salt,operator_binding_object_hash) VALUES(0,?1,?2,?3,?4,?5,?6)",&[V::Blob(fixture.head.active_certificate_fields(fixture.native.certificate).unwrap().organization_id.as_bytes().to_vec()),V::Blob(vec![0x73;16]),V::Text("Destruction Writer".into()),V::Text("Operator".into()),V::Blob(vec![0x74;32]),V::Blob(fixture.native.binding.as_bytes().to_vec())]).unwrap();
    let binding = repository.reserve_evidence_draft(source_of(&fixture)).unwrap();
    ...
    assert!(matches!(&result, Err(ea_writer::WriterError::Draft(ea_draft::DraftError::EvidenceBinding))),
        "a scoped repository must not turn an Evidence draft into an ordinary Incident; actual refusal: {:?}", result.as_ref().err().map(|error|error.code()));
    let after = repository.load_or_create().unwrap();
    assert!(after.draft_id() == id);
...
fn evidence_discard_uses_the_existing_irreversible_path_and_keeps_the_job() {
    ...
    let ordinary = Arc::new(AutosaveDraftRepository::new(fixture.native.database.clone(),provider.clone()));
    let binding = ordinary.reserve_evidence_draft(source_of(&fixture)).unwrap();
    let exact = Arc::new(ordinary.for_evidence_draft(binding).unwrap());
    let proof = || fixture.native.proof(&fixture.head,ea_operator::ReauthPurpose::DiscardDraft);
    let unbound = ea_draft::DiscardService::new(ordinary.clone(),provider.clone(),fixture.native.binding,fixture.head.preexisting_effective_now());
    assert!(matches!(unbound.begin_discard(proof()),Err(ea_draft::DraftError::EvidenceReserved)));
    assert!(!provider.deleted_draft.load(std::sync::atomic::Ordering::SeqCst));
    let service = ea_draft::DiscardService::new(exact,provider.clone(),fixture.native.binding,fixture.head.preexisting_effective_now());
    ...
    assert_eq!(fixture.native.database.query_row("SELECT count(*) FROM destruction_job",&[]).unwrap().unwrap().integer(0).unwrap(),1);
}
...
fn evidence_reservation_refuses_a_foreign_job_hash_and_existing_transition() {
    ...
    let foreign = ea_draft::EvidenceDraftSource::new(source.organization_id(),source.destruction_id(),source.authorization_hash(),ea_crypto::object_hash(b"other preflight"));
    assert!(matches!(repository.reserve_evidence_draft(foreign),Err(ea_draft::DraftError::EvidenceBinding)));
    ...
    assert!(matches!(repository.reserve_evidence_draft(source),Err(ea_draft::DraftError::EvidenceOccupied)));
...
fn missing_registered_evidence_table_cannot_reopen_a_reserved_slot_as_blank() {
    ...
    ordinary.reserve_evidence_draft(source_of(&fixture)).unwrap();
    fixture.native.database.execute("DROP TABLE writer_evidence_draft",&[]).unwrap();
    assert!(ordinary.load_or_create().is_err(), "registered Evidence schema missing is corruption, not an unreserved empty draft");
}
