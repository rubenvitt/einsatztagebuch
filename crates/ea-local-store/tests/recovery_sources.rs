...
fn source_capture_restore_binding_and_final_report_are_separate_durable_sources() {
    ...
    database
        .execute(
            "INSERT INTO recovery_test_failure VALUES(?1,?1,?1,?2,?3,100)",
            &[hash.clone(), exact.clone(), event.clone()],
        )
        .unwrap();
    assert!(
        database
            .execute("UPDATE recovery_test_failure SET failed_at=101", &[])
            .is_err()
    );
    assert!(
        database
            .execute("DELETE FROM recovery_test_failure", &[])
            .is_err()
    );
    drop(database);
    ...
    assert_eq!(
        reopened
            .query_row("SELECT exact_report FROM recovery_test_failure", &[])
            .unwrap()
            .unwrap()
            .blob(0)
            .unwrap(),
