...
fn historical_prefix_snapshot(path: &std::path::Path, count: usize) -> ([u8; 32], [u8; 32]) {
    ...
    connection
        .execute_batch(&format!(r#"PRAGMA key = "x'{}'";"#, "5b".repeat(32)))
        .unwrap();
    ...
    for migration in &ea_local_store::migrations::MIGRATIONS[..count] {
        ...
        connection
            .execute(
                "INSERT INTO schema_migration VALUES(?1,?2,0)",
    ...
    (
        *ea_crypto::object_hash(&fs::read(path).unwrap()).as_bytes(),
        *ea_crypto::object_hash(&chain).as_bytes(),
    )
}
...
fn known_historical_migration_prefix_restores_exactly_without_upgrading_sources() {
    ...
    let restored = EncryptedDatabase::restore_with_backup_key(
        ...
        ciphertext,
        migrations,
        &ea_crypto::SecretBytes::new([0x5b; 32]),
        &target,
        &provider,
        &target_key,
    )
    .expect(
        "the exact signed known migration prefix must remain a usable read-only recovery source",
    );
    assert!(restored.has_migration(18).unwrap());
    assert!(
        !restored.has_migration(19).unwrap(),
        "restoration must not silently apply a later migration"
    );
    assert_eq!(
        restored
            .query_row("SELECT key_bytes FROM incident_number_retained_key", &[])
    ...
    assert!(
        restored
            .execute("CREATE TABLE unauthorized_write(value TEXT)", &[])
            .is_err(),
        "an exact recovery source handle is read-only"
    );
    let digest = restored.recovery_source_content_hash().unwrap();
    drop(restored);
    let exact_target = fs::read(&target).unwrap();
    let reopened =
        EncryptedDatabase::open_recovery_source_exact(&target, migrations, &provider, &target_key)
            .unwrap();
