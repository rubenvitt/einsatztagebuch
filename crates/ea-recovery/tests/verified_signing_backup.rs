...
use ea_recovery::{ContainedKeyKind, EncryptedKeyContainer, RecoveryError, seal_verified_signing, verify_signing_container_key};
...
fn verified_signing_container_checks_actual_key_without_changing_bytes() {
    ...
    let container = EncryptedKeyContainer::seal(
        ContainedKeyKind::Signing, SecretBytes::new([0x73; 32]), &passphrase,
    ).unwrap();
    let original = container.to_bytes();
    let reopened = EncryptedKeyContainer::from_bytes(&original).unwrap();
    assert_eq!(verify_signing_container_key(&reopened, &expected, &passphrase), Ok(()));
    assert_eq!(reopened.to_bytes(), original);
    assert_eq!(verify_signing_container_key(&reopened, &public(0x74), &passphrase), Err(RecoveryError::KeySource));
    assert_eq!(verify_signing_container_key(&reopened, &expected, &SecretVec::new(b"wrong".to_vec())), Err(RecoveryError::ContainerOpen));
    let mut changed = original;
    *changed.last_mut().unwrap() ^= 1;
    let changed = EncryptedKeyContainer::from_bytes(&changed).unwrap();
    assert_eq!(verify_signing_container_key(&changed, &expected, &passphrase), Err(RecoveryError::ContainerOpen));
}
...
fn verified_signing_container_checks_kind_and_bounds_before_open() {
    ...
    let signing = EncryptedKeyContainer::seal(
        ContainedKeyKind::Signing, SecretBytes::new([0x75; 32]), &passphrase,
    ).unwrap();
    let recipient = EncryptedKeyContainer::seal(
        ContainedKeyKind::RecipientKem, SecretBytes::new([0x75; 32]), &passphrase,
    ).unwrap();
    let empty = SecretVec::new(Vec::new());
    assert_eq!(verify_signing_container_key(&recipient, &expected, &empty), Err(RecoveryError::KeySource));
    assert_eq!(verify_signing_container_key(&signing, &CanonicalPublicCoseKey::X25519([0x75; 32]), &empty), Err(RecoveryError::KeySource));
    assert_eq!(verify_signing_container_key(&signing, &expected, &empty), Err(RecoveryError::SecretEmpty));
    assert_eq!(verify_signing_container_key(&signing, &expected, &SecretVec::new(vec![b'p'; ea_recovery::MAX_SECRET_FILE_BYTES_V1 + 1])), Err(RecoveryError::KeySource));
    assert_eq!(verify_signing_container_key(&signing, &expected, &passphrase), Ok(()));
}
