...
mod epochs;
fn signed_source_case(retired: bool, rotated_root:bool, rotated_kem:bool, rotated_writer:bool, pkcs11:bool) {
    use ea_format::*;
    use ea_recovery::{
    ...
    let replacement_signer=ea_crypto::CoseSigner::from_secret(ea_crypto::SecretBytes::new([0x87;32]));
    let retired_medium = if retired {
        let before = f.selected(1,800,800);
        let (hash,fields) = before.active_certificates().find(|(_,c)|c.certificate_kind==CertificateKindV1::ServerReceipt).unwrap();
        let fields=fields.clone();
        let opts=||trust_support::HeadOptions{effective_from:Some(1),valid_through:Some(100),not_after:UnixMillis::new(10_000),..Default::default()};
        f.line.push(trust_support::ActionSpec::Revoke{target_kind:2,object_hash:ObjectHash::try_from(hash.as_bytes().as_slice()).unwrap()},opts());
        f.line.push(trust_support::ActionSpec::Device{kind:CertificateKindV1::ServerReceipt,marker:0x76,effective_from:Some(1)},trust_support::HeadOptions{signing_public_key_override:Some(replacement_signer.public_key().unwrap()),..opts()});
        assert!(fields.signing_key_thumbprint.unwrap()!=replacement_signer.public_key().unwrap().thumbprint());
        Some((hash,fields))
    } else {None};
    f.head = f.line.push(
    ...
    if rotated_root {
        f.head=f.line.push(trust_support::ActionSpec::RootRotate{previous_root_hash:None,effective_version:None},trust_support::HeadOptions{effective_from:Some(1),valid_through:Some(100),not_after:UnixMillis::new(10_000),..Default::default()});
    }
    let epoch=rotated_kem.then(||epochs::append_kem_epoch(&mut f,writer_binding.unwrap(),rotated_writer,None));
    ...
    if rotated_root {
        assert!(head.root_certificate_fields().root_key_thumbprint!=f.anchor.root_key_thumbprint());
    ...
    if let Some(token)=&token {
        for row in &mut rows {
            ...
            if row["keyRole"]=="recoveryRecipient" {row["protectionProfile"]=serde_json::json!("pkcs11");}
        }
    }
    ...
    let make_core = |keys: &KeyInventory| {
        ...
        value.tip_entry_hash = *epoch.as_ref().map_or(f.entry_hash,|epoch|epoch.entry_hash).as_bytes();
        value.probes = keys
            .media()
            .iter()
            ...
            .map(|m| RecoveryProbeBinding {
                ...
                setup_entry_hash: *epoch.as_ref().filter(|epoch|m.certificate()==epoch.new_certificate).map_or(f.entry_hash,|epoch|epoch.entry_hash).as_bytes(),
                initial_grant_hash: *ea_crypto::object_hash(epoch.as_ref().filter(|epoch|m.certificate()==epoch.new_certificate).map_or(f.original_bytes.as_slice(),|epoch|epoch.grant_bytes.as_slice())).as_bytes(),
            })
            .collect();
    ...
    if rotated_root {
        for missing in ["root","root-current"] {
            let subset=rows.iter().filter(|row|row["mediumId"]!=missing).cloned().collect::<Vec<_>>();
            ...
            assert!(matches!(verify_recovery_source(&exact,&source,&f.anchor,&subset,UnixMillis::new(800)),Err(ea_recovery::RecoveryTestError::Incomplete)),"missing accepted Root epoch must be incomplete");
        }
    }
    if let Some(epoch)=&epoch {
        for missing in [epoch.old_certificate,epoch.new_certificate] {
            let subset=rows.iter().filter(|row|row["certificateObjectHash"]!=hex::encode(missing.as_bytes())).cloned().collect::<Vec<_>>();
            let subset=KeyInventory::parse(&inventory_bytes(&subset)).unwrap();
            let exact=signed(&make_core(&subset),LocalAuditOutcomeV1::Accepted);
            assert!(matches!(verify_recovery_source(&exact,&source,&f.anchor,&subset,UnixMillis::new(800)),Err(ea_recovery::RecoveryTestError::Incomplete)),"a missing existing KEM epoch must remain incomplete");
        }
    }
    let new_run = || {
    ...
    assert_eq!(failed_json["media"].as_array().unwrap().len(),keys.media().len());
    assert!(failed_json["media"].as_array().unwrap().iter().all(|m|m["result"]=="missing"));
    let failure_core=ea_recovery::RecoveryFailureCore::new(&missing,[93;32],UnixMillis::new(800)).unwrap();
    let signed_failure=|failure_core:&ea_recovery::RecoveryFailureCore,outcome| {
        ...
        let exact=encode_local_audit_core(&LocalAuditEventCoreFieldsV1{
            event_id:EventId::try_from(&[0x65;16][..]).unwrap(),organization_id:f.anchor.organization_id(),
            device_id:head.active_certificate_fields(cert).unwrap().device_id,
            operator_binding_object_hash:Some(f.operator_binding),signer_certificate_object_hash:f.line.second_bootstrap_admin_hash(),
            action:LocalAuditActionV1::RecoveryTest(GenericAuditContextV1::new(Some(ObjectHash::from(failure_core.context_hash())))),
            outcome,effective_now:UnixMillis::new(800),nonce:[0x66;32],
        }).unwrap();
        let signature=ea_crypto::CoseSigner::from_secret(ea_crypto::SecretBytes::new(trust_support::second_admin_signing_secret())).sign_local_audit(&exact).unwrap();
        ea_recovery::recovery_failure_envelope(&failure_core,&encode_local_audit_event(&exact,&signature).unwrap()).unwrap()
    };
    let exact_failure=signed_failure(&failure_core,LocalAuditOutcomeV1::Failed);
    let checked_failure=ea_recovery::verify_failed_recovery_report(&exact_failure,&verified,&source,&f.anchor,&keys,UnixMillis::new(800)).unwrap();
    assert_eq!(checked_failure.exact_envelope(),exact_failure);
    assert_eq!(checked_failure.public_report(),missing.exact_report());
    assert!(ea_recovery::verify_completed_recovery_report(&exact_failure,&verified,&source,&f.anchor,&keys,UnixMillis::new(800)).is_err());
    assert!(ea_recovery::verify_failed_recovery_report(&signed_failure(&failure_core,LocalAuditOutcomeV1::Completed),&verified,&source,&f.anchor,&keys,UnixMillis::new(800)).is_err());
    let mut changed=exact_failure.clone();let last=changed.len()-1;changed[last]^=1;
    assert!(ea_recovery::verify_failed_recovery_report(&changed,&verified,&source,&f.anchor,&keys,UnixMillis::new(800)).is_err());
    let mut run = new_run();
    ...
    if rotated_root {
        ...
        secrets.push(hex::decode("f5e5767cf153319517630f226876b86c8160cc583bc013744c6bf255f5cc0ee5").unwrap().try_into().unwrap());
    }
    if retired {secrets.push([0x87;32]);}
    if rotated_writer {secrets.push([0x88;32]);}
    let signers=secrets.into_iter()
    ...
    for medium in keys.media() {
        if medium.role() == RecoveryKeyRole::RecoveryRecipient {
            let key=if epoch.as_ref().is_some_and(|epoch|epoch.new_certificate==medium.certificate()) {fixture::other_recipient_private_key()} else {fixture::complete_recipient_private_key()};
            let key:&dyn ea_crypto::HpkeRecipient=token.as_ref().map_or(&key as &dyn ea_crypto::HpkeRecipient,|token|&token.recovery);
            run.test_recovery(medium.pseudonymous_id_hash(),key).unwrap();
            failed_run.test_recovery(medium.pseudonymous_id_hash(),key).unwrap();
        } else {
            let signer = signers
                .iter()
                .find(|signer| {
                ...
                .unwrap();
            let signer:&dyn ea_recovery::RecoverySigningBackup=token.as_ref().filter(|token|token.signing.public_key().unwrap().thumbprint()==medium.expected_thumbprint()).map_or(signer as &dyn ea_recovery::RecoverySigningBackup,|token|&token.signing);
    ...
    let ea_recovery::RecoveryRunOutcome::Failed(failed)=failed_run.finish_report().unwrap()
        else {panic!("a failed key test cannot be erased by retrying with a good key");};
    let failed_json:serde_json::Value=serde_json::from_slice(failed.exact_report()).unwrap();
    assert_eq!(failed_json["errorCode"],ea_recovery::RecoveryTestError::Incomplete.code());
    assert_eq!(failed_json["media"].as_array().unwrap().iter().filter(|row|row["result"]=="failed").count(),1);
    let wrong=failed_json["media"].as_array().unwrap().iter().find(|row|row["result"]=="failed").unwrap();
    assert_eq!(wrong["mediumIdHash"],hex::encode(root_medium.pseudonymous_id_hash().as_bytes()));
    assert_eq!(wrong["errorCode"],ea_recovery::RecoveryTestError::Key.code());
    assert_ne!(wrong["observedThumbprint"],wrong["expectedThumbprint"]);
    let failed_core=ea_recovery::RecoveryFailureCore::new(&failed,[93;32],UnixMillis::new(800)).unwrap();
    let checked=ea_recovery::verify_failed_recovery_report(
        &signed_failure(&failed_core,LocalAuditOutcomeV1::Failed),&verified,&source,&f.anchor,&keys,UnixMillis::new(800),
    ).unwrap();
    assert_eq!(checked.public_report(),failed.exact_report());
    let ea_recovery::RecoveryRunOutcome::Completed(complete)=run.finish_report().unwrap()
        else {panic!("every actual medium and sample was complete");};
    ...
    assert_eq!(report["samples"].as_array().unwrap().len(), if rotated_kem {2}else{1});
    if rotated_writer {
        let samples=report["samples"].as_array().unwrap();
        assert_ne!(samples[0]["writerCertificateHash"],samples[1]["writerCertificateHash"]);
    ...
    let completion=ea_recovery::RecoveryCompletionCore::new(&complete,[93;32],UnixMillis::new(800),UnixMillis::new(800+head.policy_fields().restore_test_interval_ms as i64)).unwrap();
    let completion_audit=|outcome| {
        let exact=encode_local_audit_core(&LocalAuditEventCoreFieldsV1 {
            event_id:EventId::try_from(&[94;16][..]).unwrap(),
            organization_id:f.anchor.organization_id(),
            device_id:head.active_certificate_fields(CertificateHash::from(f.line.second_bootstrap_admin_hash())).unwrap().device_id,
            operator_binding_object_hash:Some(f.operator_binding),
            signer_certificate_object_hash:f.line.second_bootstrap_admin_hash(),
            action:LocalAuditActionV1::RecoveryTest(GenericAuditContextV1::new(Some(ObjectHash::from(completion.context_hash())))),
            outcome,effective_now:UnixMillis::new(800),nonce:[95;32],
        }).unwrap();
        let cose=ea_crypto::CoseSigner::from_secret(ea_crypto::SecretBytes::new(trust_support::second_admin_signing_secret())).sign_local_audit(&exact).unwrap();
        ea_recovery::recovery_completion_envelope(&completion,&encode_local_audit_event(&exact,&cose).unwrap()).unwrap()
    };
    let exact_completion=completion_audit(LocalAuditOutcomeV1::Completed);
    let checked=ea_recovery::verify_completed_recovery_report(&exact_completion,&verified,&source,&f.anchor,&keys,UnixMillis::new(800)).unwrap();
    assert_eq!(checked.restored_content_hash(),&[93;32]);
    assert_eq!(checked.exact_envelope(),exact_completion);
    assert!(ea_recovery::verify_completed_recovery_report(&completion_audit(LocalAuditOutcomeV1::Accepted),&verified,&source,&f.anchor,&keys,UnixMillis::new(800)).is_err());
    for needle in [&[93;32][..],complete.exact_report()] {
        let mut changed=exact_completion.clone();let at=changed.windows(needle.len()).position(|b|b==needle).unwrap();changed[at]^=1;
        assert!(ea_recovery::verify_completed_recovery_report(&changed,&verified,&source,&f.anchor,&keys,UnixMillis::new(800)).is_err());
    }
...
fn exact_source_core_binds_all_public_source_and_epoch_inputs() {
    ...
    assert!(RecoverySourceCore::new(gap).is_err(), "a signed source cannot invent an unobserved next sequence");
...
fn source_scope_preserves_staging_while_public_verification_keeps_the_committed_tip() {
    let f=support::verify_support::historical::fixture_with_payload(recovery_fixture::recovery_payload);
    let temp=support::temp_dir("source-scope-staging");support::materialize(&f.fixture,temp.path());
    let before=ea_recovery::FsArchiveSource::open(temp.path()).unwrap();let original_hash=ea_recovery::recovery_archive_inventory_hash(&before).unwrap();
    std::fs::write(temp.path().join("entries/prepared.eip.staging"),&f.entry_bytes).unwrap();
    let frozen=ea_recovery::FsArchiveSource::open(temp.path()).unwrap();
    let whole_hash=ea_recovery::recovery_archive_inventory_hash(&frozen).unwrap();assert_ne!(whole_hash,original_hash);
    let probe=ea_recovery::RecoveryArchiveProbe::verify(&frozen,&f.anchor,ea_types::UnixMillis::new(800)).unwrap();
    assert_eq!(probe.inventory().entries().len(),1);
    std::fs::remove_file(temp.path().join("entries/prepared.eip.staging")).unwrap();
    assert_eq!(ea_recovery::recovery_archive_inventory_hash(&frozen).unwrap(),whole_hash,"the signed scope keeps its exact snapshot");
    assert_eq!(ea_recovery::recovery_archive_inventory_hash(&ea_recovery::FsArchiveSource::open(temp.path()).unwrap()).unwrap(),original_hash);
}
