...
use ea_archive::{ArchiveBackend, ArchiveBackendError, ArchiveBlob, ArchiveError, ArchivePath, ArchiveSource, LAYOUT_PATHS_V1, MAX_ARCHIVE_BLOBS_V1, MAX_TOTAL_ARCHIVE_BYTES_V1, WriterLock, WriterLockRelease};
use ea_format::ExactObjectBytes;
use ea_local_store::{StoreError, StoreTransaction, StoreValue};
use std::{fs::{self, File, OpenOptions}, path::PathBuf, sync::Arc};
...
impl SqlcipherArchiveBackend {
    pub fn open(store: SqliteCommitStore) -> Result<Self, ArchiveBackendError> {
        if !store.database().has_migration(26).map_err(|_| ArchiveBackendError::Io)? {
            return Err(ArchiveBackendError::MissingLocalCommitComponent);
        }
        let database_path = fs::canonicalize(store.database().path()).map_err(|_| ArchiveBackendError::Io)?;
        let backend = Self { store, database_path };
        backend.transaction(|_| Ok(()))?;
        Ok(backend)
    }
    fn transaction<T>(&self, work: impl FnOnce(&StoreTransaction<'_>) -> Result<T, StorageFailure>) -> Result<T, ArchiveBackendError> {
        self.store.database().transaction(|tx| {
            require_durability(tx)?;
            work(tx)
        }).map_err(|e: StorageFailure| e.0)
    }
    fn namespace(&self) -> StoreValue { StoreValue::Blob(self.store.namespace().as_bytes().to_vec()) }
    fn ensure_directory(&self, tx: &StoreTransaction<'_>, directory: &str) -> Result<(), StorageFailure> {
        ...
        tx.execute("INSERT OR IGNORE INTO local_commit_directory(namespace,directory) VALUES(?1,?2)", &[self.namespace(), StoreValue::Text(directory.to_owned())])?;
        Ok(())
    }
    ...
    fn flush_physical(&self) -> Result<(), StorageFailure> {
        ...
        File::open(&self.database_path).and_then(|f| f.sync_all()).map_err(|_| StorageFailure(ArchiveBackendError::FlushFailed))?;
        let mut wal = self.database_path.as_os_str().to_os_string(); wal.push("-wal");
        match File::open(PathBuf::from(wal)) {
            Ok(file) => file.sync_all().map_err(|_| StorageFailure(ArchiveBackendError::FlushFailed))?,
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => {},
            Err(_) => return Err(StorageFailure(ArchiveBackendError::FlushFailed)),
        }
        let parent = self.database_path.parent().ok_or(StorageFailure(ArchiveBackendError::FlushFailed))?;
        File::open(parent).and_then(|f| f.sync_all()).map_err(|_| StorageFailure(ArchiveBackendError::FlushFailed))
    }
    fn flush(&self) -> Result<(), ArchiveBackendError> { self.transaction(|_| self.flush_physical()) }
...
fn require_durability(tx: &StoreTransaction<'_>) -> Result<(), StorageFailure> {
    let synchronous = tx.query_row("PRAGMA synchronous", &[])?.ok_or(StoreError::Shape)?;
    let journal = tx.query_row("PRAGMA journal_mode", &[])?.ok_or(StoreError::Shape)?;
...
impl WriterLockRelease for SqlcipherWriterLock { fn release(&self) { let _ = self.0.unlock(); } }
impl ArchiveBackend for SqlcipherArchiveBackend {
    fn create_if_absent(&self, path: &ArchivePath, bytes: &ExactObjectBytes) -> Result<(), ArchiveBackendError> { self.create(path, bytes.as_bytes()) }
    fn create_non_object_if_absent(&self, path: &ArchivePath, bytes: &[u8]) -> Result<(), ArchiveBackendError> { self.create(path, bytes) }
    fn staged_paths(&self) -> Result<Vec<String>, ArchiveBackendError> { Ok(self.snapshot()?.into_iter().filter_map(|(path, _)| ea_archive::is_staging_path(&path).then_some(path)).collect()) }
    fn visit_managed_blobs(&self, visitor: &mut dyn FnMut(ArchiveBlob<'_>) -> Result<(), ArchiveError>) -> Result<(), ArchiveError> {
        for (path, bytes) in self.snapshot().map_err(|_| ArchiveError::Unavailable)? { visitor(ArchiveBlob::new(&path, &bytes))?; }
        Ok(())
    }
    fn remove_if_present(&self, path: &ArchivePath) -> Result<(), ArchiveBackendError> {
        let params = self.store.params(path.as_str())?;
        self.transaction(|tx| { tx.execute("DELETE FROM local_commit_object WHERE namespace=?1 AND relative_path=?2", &params)?; Ok(()) })?;
        self.flush()
    }
    fn sync_file(&self, path: &ArchivePath) -> Result<(), ArchiveBackendError> {
        let params = self.store.params(path.as_str())?;
        self.transaction(|tx| {
            if tx.query_row("SELECT 1 FROM local_commit_object WHERE namespace=?1 AND relative_path=?2", &params)?.is_none() { return Err(StorageFailure(ArchiveBackendError::FlushFailed)); }
            self.flush_physical()
        })
    }
    fn sync_directory(&self, path: &ArchivePath) -> Result<(), ArchiveBackendError> {
        ...
        self.transaction(|tx| {
            if !path.directory().is_empty() && tx.query_row("SELECT 1 FROM local_commit_directory WHERE namespace=?1 AND directory=?2", &[self.namespace(), StoreValue::Text(path.directory().to_owned())])?.is_none() { return Err(StorageFailure(ArchiveBackendError::FlushFailed)); }
            self.flush_physical()
        })
    }
    fn atomic_rename_same_fs(&self, from: &ArchivePath, to: &ArchivePath) -> Result<(), ArchiveBackendError> {
        let source = self.store.params(from.as_str())?; let target = self.store.params(to.as_str())?;
        self.transaction(|tx| {
    ...
    fn create_directory_if_absent(&self, directory: &str) -> Result<(), ArchiveBackendError> {
        if directory.is_empty() { return Err(ArchiveBackendError::Path); }
        self.transaction(|tx| self.ensure_directory(tx, directory))?; self.flush()
    }
    fn acquire_writer_lock(&self) -> Result<WriterLock, ArchiveBackendError> {
        ...
        let mut name = self.database_path.as_os_str().to_os_string(); name.push(".archive-writer.lock");
        let path = PathBuf::from(name);
        let file = OpenOptions::new().write(true).create(true).truncate(false).open(&path).map_err(|_| ArchiveBackendError::AlreadyLocked)?;
        if !fs::symlink_metadata(&path).map_err(|_| ArchiveBackendError::AlreadyLocked)?.file_type().is_file() { return Err(ArchiveBackendError::AlreadyLocked); }
        file.try_lock().map_err(|_| ArchiveBackendError::AlreadyLocked)?;
        Ok(WriterLock::new(Arc::new(SqlcipherWriterLock(file))))
    }
}
impl ArchiveSource for SqlcipherArchiveBackend {
    fn visit_blobs(&self, visitor: &mut dyn FnMut(ArchiveBlob<'_>) -> Result<(), ArchiveError>) -> Result<(), ArchiveError> {
        for (path, bytes) in self.snapshot().map_err(|_| ArchiveError::Unavailable)? {
            if !ea_archive::is_staging_path(&path) { visitor(ArchiveBlob::new(&path, &bytes))?; }
        }
        Ok(())
    }
}
