...
impl EvidenceDraftSource {
    ...
    pub const fn preflight_hash(&self) -> ObjectHash {
        self.preflight_hash
    }
}
#[derive(Clone, Copy, Eq, PartialEq)]
...
impl EvidenceDraftBinding {
    ...
    pub const fn source(&self) -> EvidenceDraftSource {
        self.source
    }
    pub const fn destruction_id(&self) -> DestructionId {
        self.source.destruction_id
    }
    pub const fn authorization_hash(&self) -> ObjectHash {
        self.source.authorization_hash
    }
    pub const fn preflight_hash(&self) -> ObjectHash {
        self.source.preflight_hash
    }
}
...
) -> Result<Option<EvidenceDraftBinding>, DraftError> {
    if tx
        .query_row(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='writer_evidence_draft'",
            &[],
        )?
        .is_none()
    {
        if tx
            .query_row("SELECT version FROM schema_migration WHERE version=25", &[])?
            .is_some()
        {
...
fn require_job(tx: &StoreTransaction<'_>, source: EvidenceDraftSource) -> Result<(), DraftError> {
    ...
    if row.blob(0)? != source.authorization_hash.as_bytes()
        || object_hash(row.blob(1)?) != source.preflight_hash
    {
        return Err(DraftError::EvidenceBinding);
    }
    Ok(())
...
impl AutosaveDraftRepository {
    ...
    pub fn for_evidence_draft(&self, binding: EvidenceDraftBinding) -> Result<Self, DraftError> {
        if self.evidence_binding()? != Some(binding) {
            return Err(DraftError::EvidenceBinding);
        }
        Ok(Self {
            database: self.database.clone(),
            provider: self.provider.clone(),
            evidence: Some(binding),
        })
    }
    pub(crate) fn require_evidence_access(
        &self,
        tx: &StoreTransaction<'_>,
    ) -> Result<(), DraftError> {
    ...
    pub(crate) fn check_evidence_access(&self) -> Result<(), DraftError> {
        self.database
            .transaction(|tx| self.require_evidence_access(tx))
    }
}
