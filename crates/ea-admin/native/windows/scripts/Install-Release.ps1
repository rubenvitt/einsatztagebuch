...
# Explicit fresh protected installation; never run automatically or by fixture CI.
[CmdletBinding()]
param(
...
try {
    if (-not [Security.Principal.WindowsPrincipal]::new($identity).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) { throw 'Elevated deployment administrator required' }
} finally { $identity.Dispose() }
Initialize-EaReleaseNative
$target=[Ea.Release.Native]::FullPath($Destination)
$parent=[Ea.Release.Native]::Open([IO.Path]::GetDirectoryName($target),$false)
$source=$null; $installed=$null
try {
    Assert-EaProtectedPins $parent
    if ([IO.File]::Exists($target) -or [IO.Directory]::Exists($target)) { throw 'Fresh destination required; existing installations are never overwritten' }
    # All ten source files and every ancestor remain pinned through verification,
    # copy and target re-verification. No unverified executable is copied.
    # Authenticode's path port is used only after the source's effective owner/
    # DACL and every ancestor are protected too. Sharing flags alone do not
    # prevent an untrusted owner changing attributes or security descriptors.
    $source=Open-EaReleaseBundle $Bundle $Architecture $Version $certificateDer -RequireProtected
    [Ea.Release.Native]::CreateProtectedDirectory($target)
    foreach ($name in (@(Get-EaReleaseAssets)+@('ea-native-operator.release.psd1'))) {
        [Ea.Release.Native]::CopyNewProtectedFile($source.Files[$name].Stream,(Join-Path $target $name))
    }
    $installed=Open-EaReleaseBundle $target $Architecture $Version $certificateDer -RequireProtected
    Write-Output 'Verified release installed in a fresh protected directory. Native Windows acceptance remains required.'
} catch {
    # Never recursively remove a path on failure. An incomplete fresh destination
    # is not accepted by the parent; the deployment operator inspects it manually.
    throw
} finally {
    if ($null -ne $installed) { Close-EaReleaseBundle $installed }
    if ($null -ne $source) { Close-EaReleaseBundle $source }
    $parent.Dispose()
}
