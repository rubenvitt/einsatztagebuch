//! Laufzeitnachweis nach
//! `docs/superpowers/specs/2026-08-15-einsatzarchiv-web-reader-design.md` §14.1.
//!
//! Vier Elemente, die AUSGEFUEHRT werden muessen, nicht nur uebersetzt:
//!
//! 1. wasm-bindgen-Schicht  -> [`run_runtime_proof`], `#[wasm_bindgen]`-exportiert.
//! 2. getrandom/wasm_js     -> [`prove_getrandom`], zwei Ziehungen plus ein
//!                             hpke_seal/hpke_open-Rundlauf mit frischer Entropie.
//! 3. HPKE-Entkapselung     -> [`prove_hpke`], gegen die eingefrorenen Bytes von
//!                             `vectors/crypto/suite-1/hpke/base-mode-wrapped-cek.bin`.
//! 4. Signaturpruefung      -> [`prove_ed25519`], gegen
//!                             `vectors/crypto/suite-1/ed25519/rfc8032-test1.bin`
//!                             UND gegen die verfaelschte `flipped-signature.bin`.
//!
//! Die Erwartungswerte sind NICHT neu hergeleitet. Sie stammen aus
//! `vectors/crypto/suite-1/manifest.json` bzw. aus den Konstanten von
//! `ea-testkit`, die der native Test
//! `tests/ea-system-tests/tests/conformance_golden_vectors.rs`
//! (`check_hpke`, `check_ed25519`) benutzt. `ea-testkit` selbst ist kein
//! Abhaengiger dieses Spikes: es steht in `WASM32_EXEMPT_CRATES`
//! (`tools/xtask/src/main.rs:187`, `std::fs`-Vektorausgabe). Die Konstanten sind
//! darum woertlich mit Fundstellenangabe eingetragen.
...
// ---------------------------------------------------------------------------
// Eingefrorene Vektoren, per include_bytes! einkompiliert: das wasm-Modul
// braucht kein Dateisystem.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Minimaler JSON-Bauer. Kein serde: der Spike soll so wenig Graph wie moeglich
// zwischen sich und ea-crypto legen.
// ---------------------------------------------------------------------------
...
fn prove_getrandom() -> Result<String, String> {
    ...
    getrandom::fill(&mut large).map_err(|error| format!("getrandom::fill(100000) failed: {error}"))?;
...
fn prove_hpke() -> Result<String, String> {
    ...
    for (label, vector) in [
        ...
        let differing = vector
            ...
            .count();
        if differing != 1 {
            return Err(format!("{label} must differ in exactly one byte, not {differing}"));
        }
        let broken =
...
fn prove_ed25519() -> Result<String, String> {
    ...
    let rejection = match key.verify_ed25519_strict(ED25519_MESSAGE, ED25519_FLIPPED_SIGNATURE_VECTOR)
    {
        Ok(()) => return Err("the tampered signature MUST NOT verify, but it did".to_owned()),
        Err(error) => error.code(),
    };
    if rejection != "EA-TRUST-SIGNATURE-INVALID" {
        return Err(format!("the tampered signature was rejected with {rejection}"));
    }
...
/// Fuehrt alle vier Elemente aus und liefert einen JSON-Bericht.
///
/// Der Bericht ist IMMER wohlgeformtes JSON; ein Fehlschlag steht als
/// `"ok": false` samt `"error"` darin, damit der Node-Treiber ihn ausgeben kann,
/// statt an einem Trap zu ersticken.
#[must_use]
pub fn runtime_proof_json() -> String {
    let mut json = Json::object();
...
/// Element 1: die wasm-bindgen-Schicht. Das ist der Einstieg, den der
/// Node-Treiber aufruft.
#[cfg(target_arch = "wasm32")]
#[wasm_bindgen::prelude::wasm_bindgen]
#[must_use]
pub fn run_runtime_proof() -> String {
    runtime_proof_json()
}

/// Zweiter wasm-bindgen-Export: beweist, dass Argumente ueber die Bruecke
/// ankommen und nicht nur ein Rueckgabewert herausfaellt.
#[cfg(target_arch = "wasm32")]
#[wasm_bindgen::prelude::wasm_bindgen]
#[must_use]
pub fn echo_from_js(value: &str) -> String {
    format!("wasm received: {value}")
}
...
mod tests {
    ...
    #[test]
    fn the_native_baseline_passes_every_element() {
        let report = runtime_proof_json();
        assert!(
            report.contains("\"ok\":true"),
            "native baseline must pass: {report}"
        );
    }

    #[test]
    fn hpke_recovers_the_frozen_content_encryption_key() {
        let report = prove_hpke().expect("hpke must pass natively");
        assert!(report.contains(
            "\"recoveredContentEncryptionKey\":\"c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0\""
        ));
    }

    #[test]
    fn ed25519_accepts_the_vector_and_rejects_the_tampered_one() {
        let report = prove_ed25519().expect("ed25519 must pass natively");
        assert!(report.contains("\"acceptedValidSignature\":true"));
        assert!(report.contains("\"rejectedTamperedSignature\":true"));
        assert!(report.contains("\"tamperedRejectionCode\":\"EA-TRUST-SIGNATURE-INVALID\""));
    }
}
