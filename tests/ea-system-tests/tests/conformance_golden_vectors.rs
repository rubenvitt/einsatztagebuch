...
/// Die Zahl der Eintraege: fünfzehn angenommene, einer je Aktion, und fuenf
/// abgelehnte. Ohne diese Schranke liefe ein truncatiertes Manifest still
/// durch.
const LOCAL_AUDIT_EXPECTED_ENTRY_COUNT: usize = 20;

/// Die Zahl der Aktionen aus `schemas/reports/v1/local-audit.cddl:3`.
const LOCAL_AUDIT_ACTION_COUNT: u8 = 15;
...
fn check_local_audit_vectors(entries: &[VectorEntry]) -> usize {
    ...
    assert_eq!(
        ...
        "the accepted vectors must cover every one of the fifteen actions exactly once"
    );
    assert_eq!(
        null_binding, 1,
