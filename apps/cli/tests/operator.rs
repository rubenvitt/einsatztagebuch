...
mod process_native {
    ...
    fn native_cli_helper() {
        ...
        // Dedicated Resume diagnostic only: fixed operation ordinals and time,
        // never request data, key slots, paths, identities or response contents.
        struct ResumeHelperTrace {
            file: PathBuf,
            origin: u128,
            role: &'static str,
            ordinal: u8,
        }
        impl ResumeHelperTrace {
        ...
        if op == "watch-session" {
            ...
            // Only the explicit long recovery fixture keeps its native watcher
            // alive across the real >5min input wait; all challenge/EOF/lock
            // checks below remain identical to ordinary process fixtures.
            // Explicit Reader integration lifetime diagnosis only. Ordinary
            // fixtures retain 300s and the existing long Admin opt-in 1800s.
        ...
        bootstrap_root::signing_backup::fixture_response_returned(&directory, &request);
    }
    ...
    mod recovery {
        include!("operator_recovery/mod.rs");
    }
    ...
    mod administration {
        include!("operator_administration/mod.rs");
    }
    mod bootstrap_root {
        include!("operator_bootstrap_root/mod.rs");
    }

    mod posture {
        include!("operator_posture/mod.rs");
    }
    mod posture_document {
        include!("operator_posture_document/mod.rs");
    }
    #[cfg(feature = "desktop-fixture")]
    mod desktop {
        include!("operator_desktop/mod.rs");
    }
}
